# ensures that all passed third party dependencies are available
# sets TI_EXTRA_DEPENDENCIES_FOUND variable to TRUE/FALSE
...
# glob files to compile
# source files are set to variable ${the_target}_SRC
# include directories are set to variable ${the_target}_SRC_INCLUDE
# qt translation files are created
...
# Unified function for creating Tiger applications:
#   ti_add_app(tgt [MODULES <m1> [<m2> ...]] [EXTRAS <e1> [<e2> ...]])
#   MODULES are dependencies of the Tiger module
#   EXTRAS are dependencies of the 3rd-party libries
macro(ti_add_app the_target)
  cmake_parse_arguments(APP "" "" "MODULES;EXTRAS" ${ARGN})
  
  set(__app_modules "")
  foreach(mod ${APP_MODULES})
    list(APPEND __app_modules tiger_${mod})
  endforeach()

  ti_check_dependencies(${__app_modules})
  if(NOT TI_DEPENDENCIES_FOUND)
    message(FATAL_ERROR "Module dependencies check failed for app: ${the_target}")
    return()
  endif()
  ti_check_extra_dependencies(${APP_EXTRAS})
  if(NOT TI_EXTRA_DEPENDENCIES_FOUND)
    message(FATAL_ERROR "Extra dependencies check failed for app: ${the_target}")
    return()
  endif()
  ...
  foreach(qm_file ${QM_FILES})
	...
	add_custom_command(TARGET ${the_target} POST_BUILD
		COMMAND ${CMAKE_COMMAND} -E copy_if_different ${qm_file} ${CMAKE_CURRENT_BINARY_DIR}/lang
	)
endforeach()
  ...
  if(IS_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR}/resources)
    ...
    set(${the_target}_COPY_CONFIG_DIRS ${DO_COPY_RES} CACHE BOOL "Copy resource directory to runtime")
  ...
  install(TARGETS ${the_target} RUNTIME DESTINATION ${TIGER_BIN_INSTALL_PATH} COMPONENT dev)
endmacro()

# add applicationss from specified directories
# Usage: ti_add_app_from_directory(<location> [<extra location> ...])
  ...
  foreach(directory ${ARGN})
    if(BUILD_APPS_LIST)
      ...
      if (${_index} GREATER -1)
        ...
        message(STATUS "Skip Tiger app: ${directory}")
      endif()
    else()
      add_subdirectory(${directory})
    endif()
...
# collect applicationss from specified directories
...
macro(ti_glob_applications main_root)
  if(DEFINED TIGER_APP_INITIAL_PASS)
    message(FATAL_ERROR "Tiger has already loaded its modules. Calling ti_glob_applications second time is not
    allowed.")
  endif()
  ...
  _assert_uniqueness("Duplicated application LOCATIONS has been found" ${__app_paths})
  _assert_uniqueness("Duplicated application NAMES has been found" ${__app_names})
