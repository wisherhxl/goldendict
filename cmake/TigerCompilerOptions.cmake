...
if(ENABLE_CCACHE AND NOT TIGER_COMPILER_IS_CCACHE)
  ...
  if(CCACHE_PROGRAM)
    ...
    elseif(__OLD_RULE_LAUNCH_COMPILE)
      ...
      # ti_check_compiler_flag(CXX "" IS_CCACHE_WORKS)
...
macro(add_extra_compiler_option option)
  ti_check_flag_support(CXX "${option}" _varname "${TIGER_EXTRA_CXX_FLAGS} ${ARGN}")
  if(${_varname})
    set(TIGER_EXTRA_CXX_FLAGS "${TIGER_EXTRA_CXX_FLAGS} ${option}")
  endif()

  ti_check_flag_support(C "${option}" _varname "${TIGER_EXTRA_C_FLAGS} ${ARGN}")
  if(${_varname})
    set(TIGER_EXTRA_C_FLAGS "${TIGER_EXTRA_C_FLAGS} ${option}")
  endif()
endmacro()
...
if(TI_GCC OR TI_CLANG OR TI_ICX)
  ...
  if(ENABLE_PROFILING)
    ...
    foreach(flags CMAKE_CXX_FLAGS CMAKE_C_FLAGS CMAKE_CXX_FLAGS_RELEASE CMAKE_C_FLAGS_RELEASE CMAKE_CXX_FLAGS_DEBUG CMAKE_C_FLAGS_DEBUG
                  TIGER_EXTRA_FLAGS_RELEASE TIGER_EXTRA_FLAGS_DEBUG TIGER_EXTRA_C_FLAGS TIGER_EXTRA_CXX_FLAGS)
    ...
    elseif(((IOS OR ANDROID) AND NOT BUILD_SHARED_LIBS) AND NOT TIGER_FORCE_FUNCTIONS_SECTIONS)
  ...
  if(ENABLE_COVERAGE)
    set(TIGER_EXTRA_C_FLAGS "${TIGER_EXTRA_C_FLAGS} --coverage")
    set(TIGER_EXTRA_CXX_FLAGS "${TIGER_EXTRA_CXX_FLAGS} --coverage")
  ...
  set(TIGER_EXTRA_FLAGS_RELEASE "${TIGER_EXTRA_FLAGS_RELEASE} -DNDEBUG")
  if(NOT " ${CMAKE_CXX_FLAGS} ${CMAKE_CXX_FLAGS_DEBUG} ${TIGER_EXTRA_FLAGS_DEBUG} " MATCHES "-O")
    set(TIGER_EXTRA_FLAGS_DEBUG "${TIGER_EXTRA_FLAGS_DEBUG} -O0")
  endif()
  set(TIGER_EXTRA_FLAGS_DEBUG "${TIGER_EXTRA_FLAGS_DEBUG} -DDEBUG -D_DEBUG")

  if(BUILD_WITH_DEBUG_INFO)
    # https://gcc.gnu.org/onlinedocs/gcc/Debugging-Options.html
    # '-g' is equal to '-g2', '-g1' produces minimal information, enough for making backtraces
    ti_update(TIGER_DEBUG_OPTION "-g1")
    if(NOT " ${CMAKE_CXX_FLAGS} ${CMAKE_CXX_FLAGS_RELEASE} ${TIGER_EXTRA_CXX_FLAGS} ${TIGER_EXTRA_FLAGS_RELEASE}" MATCHES " -g")
      set(TIGER_EXTRA_FLAGS_RELEASE "${TIGER_EXTRA_FLAGS_RELEASE} ${TIGER_DEBUG_OPTION}")
    endif()
  endif()
endif()
...
if(MSVC)
  #TODO Code refactoring is required to resolve security warnings
    ...
    set(TIGER_EXTRA_FLAGS "${TIGER_EXTRA_FLAGS} /D _CRT_SECURE_NO_DEPRECATE /D _CRT_NONSTDC_NO_DEPRECATE /D _SCL_SECURE_NO_WARNINGS")
  #endif()
  ...
  # Remove unreferenced functions: function level linking
  set(TIGER_EXTRA_FLAGS "${TIGER_EXTRA_FLAGS} /Gy")
  if(NOT MSVC_VERSION LESS 1400)
    set(TIGER_EXTRA_FLAGS "${TIGER_EXTRA_FLAGS} /bigobj")
  endif()

  if(TIGER_WARNINGS_ARE_ERRORS)
    set(TIGER_EXTRA_FLAGS "${TIGER_EXTRA_FLAGS} /WX")
  endif()

  if(ENABLE_LTO)
    set(TIGER_EXTRA_FLAGS_RELEASE "${TIGER_EXTRA_FLAGS_RELEASE} /GL")
    set(TIGER_EXTRA_EXE_LINKER_FLAGS_RELEASE "${TIGER_EXTRA_EXE_LINKER_FLAGS_RELEASE} /LTCG")
  endif()
...
if(COMMAND ti_compiler_optimization_options_finalize)
  ti_compiler_optimization_options_finalize()
endif()
...
if(NOT TIGER_SKIP_LINK_AS_NEEDED)
  if(UNIX)
    ...
    ti_check_compiler_flag(CXX "" HAVE_LINK_AS_NEEDED)
...
if(NOT TIGER_SKIP_EXTRA_COMPILER_FLAGS)
  set(CMAKE_C_FLAGS           "${CMAKE_C_FLAGS} ${TIGER_EXTRA_FLAGS} ${TIGER_EXTRA_C_FLAGS}")
  set(CMAKE_CXX_FLAGS         "${CMAKE_CXX_FLAGS} ${TIGER_EXTRA_FLAGS} ${TIGER_EXTRA_CXX_FLAGS}")
  set(CMAKE_CXX_FLAGS_RELEASE "${CMAKE_CXX_FLAGS_RELEASE} ${TIGER_EXTRA_FLAGS_RELEASE}")
  set(CMAKE_C_FLAGS_RELEASE   "${CMAKE_C_FLAGS_RELEASE} ${TIGER_EXTRA_FLAGS_RELEASE}")
  set(CMAKE_CXX_FLAGS_DEBUG   "${CMAKE_CXX_FLAGS_DEBUG} ${TIGER_EXTRA_FLAGS_DEBUG}")
  set(CMAKE_C_FLAGS_DEBUG     "${CMAKE_C_FLAGS_DEBUG} ${TIGER_EXTRA_FLAGS_DEBUG}")
  set(CMAKE_EXE_LINKER_FLAGS         "${CMAKE_EXE_LINKER_FLAGS} ${TIGER_EXTRA_EXE_LINKER_FLAGS}")
  set(CMAKE_EXE_LINKER_FLAGS_RELEASE "${CMAKE_EXE_LINKER_FLAGS_RELEASE} ${TIGER_EXTRA_EXE_LINKER_FLAGS_RELEASE}")
  set(CMAKE_EXE_LINKER_FLAGS_DEBUG   "${CMAKE_EXE_LINKER_FLAGS_DEBUG} ${TIGER_EXTRA_EXE_LINKER_FLAGS_DEBUG}")
...
if(MSVC)
  if(NOT ENABLE_NOISY_WARNINGS)
    if(MSVC_VERSION EQUAL 1400)
      ti_warnings_disable(CMAKE_CXX_FLAGS /wd4510 /wd4610 /wd4312 /wd4201 /wd4244 /wd4328 /wd4267)
    endif()
  endif()
  ...
  if(NOT TIGER_SKIP_MSVC_EXCEPTIONS_FLAG)
    foreach(flags CMAKE_C_FLAGS CMAKE_C_FLAGS_RELEASE CMAKE_C_FLAGS_RELEASE CMAKE_CXX_FLAGS CMAKE_CXX_FLAGS_RELEASE CMAKE_CXX_FLAGS_DEBUG)
...
if(ENABLE_BUILD_HARDENING)
  include("${CMAKE_CURRENT_LIST_DIR}/TigerCompilerDefenses.cmake")
endif()

if(MSVC)
  include("${CMAKE_CURRENT_LIST_DIR}/TigerCRTLinkage.cmake")
  add_definitions(-D_VARIADIC_MAX=10)
endif()

if(CMAKE_SYSTEM_NAME STREQUAL "Windows")
  get_directory_property(__DIRECTORY_COMPILE_DEFINITIONS COMPILE_DEFINITIONS)
  if((NOT " ${CMAKE_CXX_FLAGS} ${CMAKE_CXX_FLAGS_RELEASE} ${TIGER_EXTRA_CXX_FLAGS} ${TIGER_EXTRA_FLAGS_RELEASE} ${__DIRECTORY_COMPILE_DEFINITIONS}" MATCHES "_WIN32_WINNT"
      AND NOT TIGER_CMAKE_SKIP_MACRO_WIN32_WINNT)
      OR TIGER_CMAKE_FORCE_MACRO_WIN32_WINNT
  )
    # https://docs.microsoft.com/en-us/cpp/porting/modifying-winver-and-win32-winnt
    # Target Windows 7 API
    set(TIGER_CMAKE_MACRO_WIN32_WINNT "0x0601" CACHE STRING "Value of _WIN32_WINNT macro")
    add_definitions(-D_WIN32_WINNT=${TIGER_CMAKE_MACRO_WIN32_WINNT})
  endif()
endif()

# Enable compiler options for Tiger modules/apps/samples only (ignore 3rdparty)
macro(ti_add_modules_compiler_options)
  if(MSVC AND NOT TIGER_SKIP_MSVC_W4_OPTION)
    foreach(flags CMAKE_CXX_FLAGS CMAKE_CXX_FLAGS_RELEASE CMAKE_CXX_FLAGS_DEBUG)
      string(REPLACE "/W3" "/W4" ${flags} "${${flags}}")
    endforeach()
  endif()
  if(TIGER_ENABLE_MEMORY_SANITIZER)
    add_definitions(-DTIGER_ENABLE_MEMORY_SANITIZER=1)
  endif()
endmacro()

# adjust -Wl,-rpath-link
if(CMAKE_SKIP_RPATH)
  if((NOT CMAKE_CROSSCOMPILING OR TIGER_ENABLE_LINKER_RPATH_LINK_ORIGIN) AND NOT TIGER_SKIP_LINKER_RPATH_LINK_ORIGIN)
  ...
  elseif(NOT TIGER_SKIP_LINKER_RPATH_LINK_BINARY_LIB)
    list(APPEND CMAKE_PLATFORM_RUNTIME_PATH "${LIBRARY_OUTPUT_PATH}")
  endif()
endif()
if(TIGER_EXTRA_RPATH_LINK_PATH)
  string(REPLACE ":" ";" TIGER_EXTRA_RPATH_LINK_PATH_ "${TIGER_EXTRA_RPATH_LINK_PATH}")
  list(APPEND CMAKE_PLATFORM_RUNTIME_PATH ${TIGER_EXTRA_RPATH_LINK_PATH_})
  if(NOT CMAKE_EXECUTABLE_RPATH_LINK_CXX_FLAG)
    message(WARNING "TIGER_EXTRA_RPATH_LINK_PATH may not work properly because CMAKE_EXECUTABLE_RPATH_LINK_CXX_FLAG is not defined (not supported)")
  endif()
endif()

# Control MSVC /MP flag
# Input variables: TIGER_MSVC_PARALLEL (ON,1,2,3,...) + TIGER_SKIP_MSVC_PARALLEL
# Details:
...
if(CMAKE_GENERATOR MATCHES "Visual Studio" AND CMAKE_CXX_COMPILER_ID MATCHES "MSVC|Intel")
  ti_check_environment_variables(TIGER_SKIP_MSVC_PARALLEL)
  if(TIGER_SKIP_MSVC_PARALLEL)
    # nothing
  ...
  elseif(DEFINED ENV{_CL_} AND " $ENV{_CL_}" MATCHES "/MP")
    ...
    ti_check_environment_variables(TIGER_MSVC_PARALLEL)
    set(_mp_value "ON")
    if(DEFINED TIGER_MSVC_PARALLEL)
      set(_mp_value "${TIGER_MSVC_PARALLEL}")
    endif()
    set(TIGER_MSVC_PARALLEL "${_mp_value}" CACHE STRING "Control MSVC /MP flag")
    if(_mp_value)
