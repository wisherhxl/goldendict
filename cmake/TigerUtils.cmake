...
#
option(TIGER_DUMP_HOOKS_FLOW "Dump called Tiger hooks" OFF)
macro(ti_cmake_hook_append hook_name)
...
macro(ti_cmake_hook hook_name)
  ...
  endforeach()
endmacro()
macro(ti_cmake_reset_hooks)
  get_cmake_property(__variableNames VARIABLES)
  foreach(__variableName ${__variableNames})
    if(__variableName MATCHES "^__TIGER_CMAKE_HOOKS_")
...
macro(ti_cmake_hook_register_dir dir)
  ...
  foreach(f ${hook_files})
    if(f MATCHES "^(.+)\\.cmake$")
      ...
      ti_cmake_hook_append(${hook_name} "${dir}/${f}")
    endif()
  endforeach()
endmacro()
...
macro(ti_cmake_configure file_name var_name)
...
endmacro()
macro(ti_declare_removed_variables)
  foreach(_var ${ARGN})
    ti_declare_removed_variable(${_var})
  endforeach()
endmacro()
...
macro(ti_debug_message)
  if(TIGER_CMAKE_DEBUG_MESSAGES)
    string(REPLACE ";" " " __msg "${ARGN}")
    message(STATUS "${__msg}")
  endif()
endmacro()

macro(ti_check_environment_variables)
  foreach(_var ${ARGN})
...
macro(ti_path_join result_var P1 P2_)
...
if(DEFINED TIGER_DEPENDANT_TARGETS_LIST)
  foreach(v ${TIGER_DEPENDANT_TARGETS_LIST})
    unset(${v} CACHE)
  endforeach()
  unset(TIGER_DEPENDANT_TARGETS_LIST CACHE)
endif()
  ...
  if(NOT DEFINED ${RESULT})
    ...
    if(_fname)
      ...
      if(${RESULT})
        ...
        foreach(_regex ${TI_COMPILER_FAIL_REGEX})
          if(NOT ${RESULT})
            break()
          endif()
...
macro(ti_check_runtime_flag flag result)
  set(_fname "${ARGN}")
  if(NOT DEFINED ${result})
...
macro(ti_append_source_file_compile_definitions source)
...
macro(add_apple_compiler_options the_module)
  ti_check_flag_support(OBJCXX "-fobjc-exceptions" HAVE_OBJC_EXCEPTIONS "")
  if(HAVE_OBJC_EXCEPTIONS)
    foreach(source ${TIGER_MODULE_${the_module}_SOURCES})
...
# Usage: ti_append_build_options(HIGHGUI FFMPEG)
macro(ti_append_build_options var_prefix pkg_prefix)
...
function(ti_append_source_files_cxx_compiler_options files_var)
  set(__flags "${ARGN}")
  ti_check_flag_support(CXX "${__flags}" __HAVE_COMPILER_OPTIONS_VAR "")
...
function(ti_install_target)
  if(APPLE_FRAMEWORK AND BUILD_SHARED_LIBS)
    install(TARGETS ${ARGN} FRAMEWORK DESTINATION ${TIGER_3P_LIB_INSTALL_PATH})
  else()
...
# ti_install_3rdparty_licenses(<library-name> <filename1> [<filename2> ..])
function(ti_install_3rdparty_licenses library)
...
macro(ti_parse_header FILENAME FILE_VAR)
  set(vars_regex "")
...
macro(ti_get_all_libs _modules _extra _3rdparty)
  ...
  set(${_extra} "")
  set(${_3rdparty} "")
  foreach(m ${TIGER_MODULES_PUBLIC})
    if(TARGET ${m})
  ...
  ti_list_filterout(${_modules} "^[\$]<")
  ti_list_filterout(${_3rdparty} "^[\$]<")
  ti_list_filterout(${_extra} "^[\$]<")
...
macro(ti_generate_vs_version_file DESTINATION)
  ...
  macro(__vs_ver_update_variable name)
    if(VS_VER_NAME AND DEFINED TIGER_${VS_VER_NAME}_VS_VER_${name})
      set(TIGER_VS_VER_${name} "${TIGER_${VS_VER_NAME}_VS_VER_${name}}")
    elseif(VS_VER_${name})
      set(TIGER_VS_VER_${name} "${VS_VER_${name}}")
    endif()
  endmacro()
  ...
  macro(__vs_ver_update_str_variable name)
    if(VS_VER_NAME AND DEFINED TIGER_${VS_VER_NAME}_VS_VER_${name})
      set(TIGER_VS_VER_${name}_STR "${TIGER_${VS_VER_NAME}_VS_VER_${name}}")
    elseif(VS_VER_${name})
      set(TIGER_VS_VER_${name}_STR "${VS_VER_${name}}")
    endif()
  endmacro()
  ...
  if(TIGER_VS_VER_COPYRIGHT_STR)
    set(TIGER_VS_VER_HAVE_COPYRIGHT_STR 1)
  else()
    set(TIGER_VS_VER_HAVE_COPYRIGHT_STR 0)
  endif()

  if(TIGER_VS_VER_COMMENTS_STR)
    set(TIGER_VS_VER_HAVE_COMMENTS_STR 1)
  else()
...
macro(ti_copyfiles_append_file list_var src dst)
...
macro(ti_copyfiles_append_dir list_var src dst)
  set(__glob ${ARGN})
  list(LENGTH ${list_var} __id)
  list(APPEND ${list_var} ${__id})
  set(${list_var}_SRC_${__id} "${src}")
...
macro(ti_copyfiles_make_config_file filename_var list_var)
  ti_copyfiles_make_config_string(${list_var}_CONFIG ${list_var})
