#
# The script to detect Intel(R) Math Kernel Library (MKL) installation/package
#
# Parameters: MKL_ROOT_DIR / ENV{MKLROOT} MKL_INCLUDE_DIR MKL_LIBRARIES
# MKL_USE_SINGLE_DYNAMIC_LIBRARY - use single dynamic library mkl_rt.lib /
# libmkl_rt.so MKL_WITH_TBB / MKL_WITH_OPENMP
#
# Extra: MKL_LIB_FIND_PATHS
#
# On return this will define:
#
# HAVE_MKL          - True if Intel MKL found MKL_ROOT_DIR      - root of MKL
# installation MKL_INCLUDE_DIRS  - MKL include folder MKL_LIBRARIES     - MKL
# libraries that are used by Tiger
#
...
macro(mkl_fail)
  ...
  set(MKL_ROOT_DIR
      "${MKL_ROOT_DIR}"
      CACHE PATH "Path to MKL directory")
  return()
endmacro()
...
macro(get_mkl_version VERSION_FILE)
  ...
  set(MKL_VERSION_STR
      ...
      CACHE STRING "MKL version" FORCE)
endmacro()

ti_option(MKL_USE_SINGLE_DYNAMIC_LIBRARY
          "Use MKL Single Dynamic Library thorugh mkl_rt.lib / libmkl_rt.so"
          OFF)
ti_option(MKL_WITH_TBB "Use MKL with TBB multithreading" OFF) # ON IF WITH_TBB)
ti_option(MKL_WITH_OPENMP "Use MKL with OpenMP multithreading" OFF
)# ON IF WITH_OPENMP)

if(NOT MKL_ROOT_DIR
   AND DEFINED MKL_INCLUDE_DIR
   AND EXISTS "${MKL_INCLUDE_DIR}/mkl.h")
...
if(NOT MKL_ROOT_DIR)
  ...
  if(WITH_MKL AND NOT mkl_root_paths)
    if(WIN32)
      ...
      list(APPEND mkl_root_paths
           ${path}/IntelSWTools/compilers_and_libraries/windows/mkl)
    endif()
    if(UNIX)
...
set(MKL_INCLUDE_DIR
    ...
    CACHE PATH "Path to MKL include directory")
...
foreach(MKL_ARCH ${MKL_ARCH_LIST})
  list(APPEND mkl_lib_find_paths ${MKL_ROOT_DIR}/lib/${MKL_ARCH}
       ${MKL_ROOT_DIR}/${MKL_ARCH})
endforeach()

if(DEFINED TIGER_MKL_LIBRARIES)
  # custom list, user specified
  set(mkl_lib_list ${TIGER_MKL_LIBRARIES})

elseif(MKL_USE_SINGLE_DYNAMIC_LIBRARY AND NOT (MKL_VERSION_STR VERSION_LESS
                                               "10.3.0"))
...
if(NOT MKL_LIBRARIES)
  ...
  endforeach()
  list(APPEND MKL_LIBRARIES ${TIGER_EXTRA_MKL_LIBRARIES})
endif()
...
if(MKL_WITH_TBB)
  if(BUILD_TBB)
    message(
      STATUS
        "MKL: reusing builtin TBB binaries is not supported. Consider disabling MKL_WITH_TBB flag to prevent build/runtime errors"
    )
  else()
...
if(UNIX
   AND NOT MKL_USE_SINGLE_DYNAMIC_LIBRARY
   AND NOT MKL_LIBRARIES_DONT_HACK)
  # it's ugly but helps to avoid cyclic lib problem
  set(MKL_LIBRARIES ${MKL_LIBRARIES} ${MKL_LIBRARIES} ${MKL_LIBRARIES}
                    "-lpthread" "-lm" "-ldl")
endif()
