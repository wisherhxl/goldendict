...
# CPU_{opt}_FLAGS_ON=""
# CPU_{opt}_FEATURE_ALIAS - mapping to TI_CPU_* HWFeature enum
...
macro(ti_optimization_process_obsolete_option legacy_flag OPT legacy_warn)
...
endmacro()
ti_optimization_process_obsolete_option(ENABLE_SSE SSE ON)
ti_optimization_process_obsolete_option(ENABLE_SSE2 SSE2 ON)
ti_optimization_process_obsolete_option(ENABLE_SSE3 SSE3 ON)
ti_optimization_process_obsolete_option(ENABLE_SSSE3 SSSE3 ON)
ti_optimization_process_obsolete_option(ENABLE_SSE41 SSE4_1 ON)
ti_optimization_process_obsolete_option(ENABLE_SSE42 SSE4_2 ON)
ti_optimization_process_obsolete_option(ENABLE_POPCNT POPCNT ON)
ti_optimization_process_obsolete_option(ENABLE_AVX AVX ON)
ti_optimization_process_obsolete_option(ENABLE_AVX2 AVX2 ON)
...
macro(ti_is_optimization_in_force_list resultvar check_opt)
  set(__checked "")
  set(__queue ${ARGN})
  set(${resultvar} 0)
  while(__queue AND NOT ${resultvar})
...
macro(ti_append_optimization_flag var OPT)
...
if(X86 OR X86_64)
  ...
  ti_update(CPU_AVX512_KNM_GROUP "AVX512_KNL;AVX512_KNM_EXTRA;AVX_512VPOPCNTDQ")
  ti_update(CPU_AVX512_SKX_GROUP "AVX512_COMMON;AVX_512VL;AVX_512BW;AVX_512DQ")
  ti_update(CPU_AVX512_CNL_GROUP "AVX512_SKX;AVX_512IFMA;AVX_512VBMI")
  ti_update(CPU_AVX512_CLX_GROUP "AVX512_SKX;AVX_512VNNI")
  ti_update(CPU_AVX512_ICL_GROUP "AVX512_SKX;AVX_512IFMA;AVX_512VBMI;AVX_512VNNI;AVX_512VBMI2;AVX_512BITALG;AVX_512VPOPCNTDQ") # ? VPCLMULQDQ, GFNI, VAES

  ti_update(CPU_SSE_TEST_FILE "${${PROJECT_NAME}_SOURCE_DIR}/cmake/checks/cpu_sse.cpp")
  ti_update(CPU_SSE2_TEST_FILE "${${PROJECT_NAME}_SOURCE_DIR}/cmake/checks/cpu_sse2.cpp")
  ti_update(CPU_SSE3_TEST_FILE "${${PROJECT_NAME}_SOURCE_DIR}/cmake/checks/cpu_sse3.cpp")
  ti_update(CPU_SSSE3_TEST_FILE "${${PROJECT_NAME}_SOURCE_DIR}/cmake/checks/cpu_ssse3.cpp")
  ti_update(CPU_SSE4_1_TEST_FILE "${${PROJECT_NAME}_SOURCE_DIR}/cmake/checks/cpu_sse41.cpp")
  ti_update(CPU_SSE4_2_TEST_FILE "${${PROJECT_NAME}_SOURCE_DIR}/cmake/checks/cpu_sse42.cpp")
  ti_update(CPU_POPCNT_TEST_FILE "${${PROJECT_NAME}_SOURCE_DIR}/cmake/checks/cpu_popcnt.cpp")
  ti_update(CPU_AVX_TEST_FILE "${${PROJECT_NAME}_SOURCE_DIR}/cmake/checks/cpu_avx.cpp")
  ti_update(CPU_AVX2_TEST_FILE "${${PROJECT_NAME}_SOURCE_DIR}/cmake/checks/cpu_avx2.cpp")
  ti_update(CPU_FP16_TEST_FILE "${${PROJECT_NAME}_SOURCE_DIR}/cmake/checks/cpu_fp16.cpp")
  ti_update(CPU_AVX_512F_TEST_FILE "${${PROJECT_NAME}_SOURCE_DIR}/cmake/checks/cpu_avx512.cpp")
  ti_update(CPU_AVX512_COMMON_TEST_FILE "${${PROJECT_NAME}_SOURCE_DIR}/cmake/checks/cpu_avx512common.cpp")
  ti_update(CPU_AVX512_KNL_TEST_FILE "${${PROJECT_NAME}_SOURCE_DIR}/cmake/checks/cpu_avx512knl.cpp")
  ti_update(CPU_AVX512_KNM_TEST_FILE "${${PROJECT_NAME}_SOURCE_DIR}/cmake/checks/cpu_avx512knm.cpp")
  ti_update(CPU_AVX512_SKX_TEST_FILE "${${PROJECT_NAME}_SOURCE_DIR}/cmake/checks/cpu_avx512skx.cpp")
  ti_update(CPU_AVX512_CNL_TEST_FILE "${${PROJECT_NAME}_SOURCE_DIR}/cmake/checks/cpu_avx512cnl.cpp")
  ti_update(CPU_AVX512_CLX_TEST_FILE "${${PROJECT_NAME}_SOURCE_DIR}/cmake/checks/cpu_avx512clx.cpp")
  ti_update(CPU_AVX512_ICL_TEST_FILE "${${PROJECT_NAME}_SOURCE_DIR}/cmake/checks/cpu_avx512icl.cpp")

  if(NOT TIGER_CPU_OPT_IMPLIES_IGNORE)
    ti_update(CPU_AVX512_ICL_IMPLIES "AVX512_SKX")
    ti_update(CPU_AVX512_CLX_IMPLIES "AVX512_SKX")
    ti_update(CPU_AVX512_CNL_IMPLIES "AVX512_SKX")
    ti_update(CPU_AVX512_SKX_IMPLIES "AVX512_COMMON")
    ti_update(CPU_AVX512_KNM_IMPLIES "AVX512_KNL")
    ti_update(CPU_AVX512_KNL_IMPLIES "AVX512_COMMON")
    ti_update(CPU_AVX512_COMMON_IMPLIES "AVX_512F")
    ti_update(CPU_AVX_512F_IMPLIES "AVX2")
    ti_update(CPU_AVX_512F_FORCE "") # Don't force other optimizations
    ti_update(CPU_AVX2_IMPLIES "AVX;FMA3;FP16")
    ti_update(CPU_FMA3_IMPLIES "AVX2")
    ti_update(CPU_FMA3_FORCE "") # Don't force other optimizations
    ti_update(CPU_FP16_IMPLIES "AVX")
    ti_update(CPU_FP16_FORCE "") # Don't force other optimizations
    ti_update(CPU_AVX_IMPLIES "SSE4_2")
    ti_update(CPU_SSE4_2_IMPLIES "SSE4_1;POPCNT")
    ti_update(CPU_POPCNT_IMPLIES "SSE4_1")
    ti_update(CPU_POPCNT_FORCE "") # Don't force other optimizations
    ti_update(CPU_SSE4_1_IMPLIES "SSE3;SSSE3")
    ti_update(CPU_SSSE3_IMPLIES "SSE3")
    ti_update(CPU_SSE3_IMPLIES "SSE2")
  ...
  if(TI_ICC OR TI_ICX)
    macro(ti_intel_compiler_optimization_option name unix_flags msvc_flags)
      ...
      ti_update(CPU_${name}_FLAGS_ON "${enable_flags}")
      if(flags_conflict)
        ti_update(CPU_${name}_FLAGS_CONFLICT "${flags_conflict}")
      endif()
    endmacro()
    ti_intel_compiler_optimization_option(AVX2 "-march=core-avx2" "/arch:CORE-AVX2")
    ...
    ti_intel_compiler_optimization_option(SSSE3 "-mssse3" "/arch:SSSE3")
    ti_intel_compiler_optimization_option(SSE2 "-msse2" "/arch:SSE2")
    if(NOT X86_64) # x64 compiler doesn't support /arch:sse
      ti_intel_compiler_optimization_option(SSE "-msse" "/arch:SSE")
    endif()
    ti_intel_compiler_optimization_option(AVX_512F "-xCOMMON-AVX512" "/Qx:COMMON-AVX512")
    ti_intel_compiler_optimization_option(AVX512_COMMON "-xCOMMON-AVX512" "/Qx:COMMON-AVX512")
    ti_intel_compiler_optimization_option(AVX512_KNL "-xKNL" "/Qx:KNL")
    ti_intel_compiler_optimization_option(AVX512_KNM "-xKNM" "/Qx:KNM")
    ti_intel_compiler_optimization_option(AVX512_SKX "-xSKYLAKE-AVX512" "/Qx:SKYLAKE-AVX512")
  ...
  elseif(TI_GCC OR TI_CLANG OR TI_ICX)
    ...
    ti_update(CPU_FMA3_FLAGS_ON "-mfma")
    ti_update(CPU_POPCNT_FLAGS_ON "-mpopcnt")
    ti_update(CPU_SSE4_2_FLAGS_ON "-msse4.2")
    ti_update(CPU_SSE4_1_FLAGS_ON "-msse4.1")
    ti_update(CPU_SSE3_FLAGS_ON "-msse3")
    ti_update(CPU_SSSE3_FLAGS_ON "-mssse3")
    ti_update(CPU_SSE2_FLAGS_ON "-msse2")
    ti_update(CPU_SSE_FLAGS_ON "-msse")
    if(NOT (TI_GCC AND CMAKE_CXX_COMPILER_VERSION VERSION_LESS "5.0"))  # GCC >= 5.0
      ti_update(CPU_AVX_512F_FLAGS_ON "-mavx512f")
      ti_update(CPU_AVX_512CD_FLAGS_ON "-mavx512cd")
      ti_update(CPU_AVX512_KNL_EXTRA_FLAGS_ON "-mavx512er -mavx512pf")
      ti_update(CPU_AVX512_KNM_EXTRA_FLAGS_ON "-mavx5124fmaps -mavx5124vnniw")
      ti_update(CPU_AVX_512BW_FLAGS_ON "-mavx512bw")
      ti_update(CPU_AVX_512DQ_FLAGS_ON "-mavx512dq")
      ti_update(CPU_AVX_512VL_FLAGS_ON "-mavx512vl")
      ti_update(CPU_AVX_512IFMA_FLAGS_ON "-mavx512ifma")
      ti_update(CPU_AVX_512VBMI_FLAGS_ON "-mavx512vbmi")
      ti_update(CPU_AVX_512VNNI_FLAGS_ON "-mavx512vnni")
      ti_update(CPU_AVX_512VBMI2_FLAGS_ON "-mavx512vbmi2")
      ti_update(CPU_AVX_512BITALG_FLAGS_ON "-mavx512bitalg")
      ti_update(CPU_AVX_512VPOPCNTDQ_FLAGS_ON "-mavx512vpopcntdq")
    else()
      ti_update(CPU_AVX_512F_SUPPORTED OFF)
    endif()
  elseif(MSVC)
    ti_update(CPU_AVX2_FLAGS_ON "/arch:AVX2")
    ti_update(CPU_AVX_FLAGS_ON "/arch:AVX")
    ti_update(CPU_FP16_FLAGS_ON "/arch:AVX")
    if(NOT X86_64)
      # 64-bit MSVC compiler uses SSE/SSE2 by default
      ti_update(CPU_SSE_FLAGS_ON "/arch:SSE")
      ti_update(CPU_SSE_SUPPORTED ON)
      ti_update(CPU_SSE2_FLAGS_ON "/arch:SSE2")
      ti_update(CPU_SSE2_SUPPORTED ON)
    else()
      ti_update(CPU_SSE_SUPPORTED ON)
      ti_update(CPU_SSE2_SUPPORTED ON)
      ti_update(CPU_AVX_512F_FLAGS_ON "/arch:AVX512")
    endif()
...
elseif(ARM OR AARCH64)
  ti_update(CPU_NEON_TEST_FILE "${${PROJECT_NAME}_SOURCE_DIR}/cmake/checks/cpu_neon.cpp")
  ti_update(CPU_FP16_TEST_FILE "${${PROJECT_NAME}_SOURCE_DIR}/cmake/checks/cpu_fp16.cpp")
  ti_update(CPU_NEON_FP16_TEST_FILE "${${PROJECT_NAME}_SOURCE_DIR}/cmake/checks/cpu_neon_fp16.cpp")
  ti_update(CPU_NEON_BF16_TEST_FILE "${${PROJECT_NAME}_SOURCE_DIR}/cmake/checks/cpu_neon_bf16.cpp")
  ti_update(CPU_NEON_DOTPROD_TEST_FILE "${${PROJECT_NAME}_SOURCE_DIR}/cmake/checks/cpu_neon_dotprod.cpp")
  if(NOT AARCH64)
    ti_update(CPU_KNOWN_OPTIMIZATIONS "VFPV3;NEON;FP16")
    if(NOT MSVC)
      ti_update(CPU_VFPV3_FLAGS_ON "-mfpu=vfpv3")
      ti_update(CPU_NEON_FLAGS_ON "-mfpu=neon")
      ti_update(CPU_NEON_FLAGS_CONFLICT "-mfpu=[^ ]*")
      ti_update(CPU_FP16_FLAGS_ON "-mfpu=neon-fp16 -mfp16-format=ieee")
      ti_update(CPU_FP16_FLAGS_CONFLICT "-mfpu=[^ ]*")
    endif()
...
elseif(MIPS)
  ...
  ti_update(CPU_MSA_TEST_FILE "${${PROJECT_NAME}_SOURCE_DIR}/cmake/checks/cpu_msa.cpp")
...
elseif(PPC64LE)
  ...
  ti_update(CPU_VSX_TEST_FILE "${${PROJECT_NAME}_SOURCE_DIR}/cmake/checks/cpu_vsx.cpp")
  ti_update(CPU_VSX3_TEST_FILE "${${PROJECT_NAME}_SOURCE_DIR}/cmake/checks/cpu_vsx3.cpp")

  if(NOT TIGER_CPU_OPT_IMPLIES_IGNORE)
    ti_update(CPU_VSX3_IMPLIES "VSX")
  endif()

  if(TI_CLANG AND (NOT ${CMAKE_CXX_COMPILER} MATCHES "xlc"))
    ti_update(CPU_VSX_FLAGS_ON "-mvsx -maltivec")
    ti_update(CPU_VSX3_FLAGS_ON "-mpower9-vector")
  else()
    ti_update(CPU_VSX_FLAGS_ON "-mcpu=power8")
    ti_update(CPU_VSX3_FLAGS_ON "-mcpu=power9 -mtune=power9")
  endif()
...
elseif(RISTI)
  ...
  ti_update(CPU_RVV_TEST_FILE "${${PROJECT_NAME}_SOURCE_DIR}/cmake/checks/cpu_rvv.cpp")
  ti_update(CPU_FP16_TEST_FILE "${${PROJECT_NAME}_SOURCE_DIR}/cmake/checks/cpu_fp16.cpp")
  ti_update(CPU_RVV_ZVFH_TEST_FILE "${${PROJECT_NAME}_SOURCE_DIR}/cmake/checks/cpu_rvv_fp16.cpp")
...
elseif(LOONGARCH64)
  ...
  ti_update(CPU_LSX_TEST_FILE "${${PROJECT_NAME}_SOURCE_DIR}/cmake/checks/cpu_lsx.cpp")
  ti_update(CPU_LASX_TEST_FILE "${${PROJECT_NAME}_SOURCE_DIR}/cmake/checks/cpu_lasx.cpp")
...
macro(ti_check_compiler_optimization OPT)
      ...
      if(CPU_${OPT}_TEST_FILE)
        ...
        if(NOT __available)
          if(NOT "x${CPU_${OPT}_FLAGS_NAME}" STREQUAL "x")
            ...
            ti_append_optimization_flag(_compile_flags ${OPT})
            ti_check_compiler_flag(CXX "${_compile_flags}" "${_varname}" "${CPU_${OPT}_TEST_FILE}")
          elseif(NOT "x${CPU_${OPT}_FLAGS_ON}" STREQUAL "x")
            ti_check_flag_support(CXX "${CPU_${OPT}_FLAGS_ON}" _varname "" "${CPU_${OPT}_TEST_FILE}")
          else()
            set(_varname "HAVE_CPU_${OPT}_SUPPORT")
            set(_compile_flags "${CPU_BASELINE_FLAGS}")
            ti_append_optimization_flag(_compile_flags ${OPT})
        ...
        ti_check_flag_support(CXX "${CPU_${OPT}_FLAGS_ON}" _varname "")
      endif()
...
if(_add_native_flag)
  ...
  ti_check_compiler_flag(CXX "-march=native" "${_varname}" "")
  if(${_varname})
    ...
    ti_check_compiler_flag(CXX "${_flag}" "${_varname}" "")
    if(${_varname})
...
foreach(OPT ${CPU_KNOWN_OPTIMIZATIONS})
  ...
  foreach(OPT2 ${CPU_BASELINE_DISABLE})
    ti_is_optimization_in_list(__is_disabled ${OPT2} ${OPT})
    if(__is_disabled)
      break()
    endif()
  endforeach()
  if(__is_disabled)
    ...
    ti_is_optimization_in_list(__is_from_baseline ${OPT} ${CPU_BASELINE_REQUIRE})
    if(NOT __is_from_baseline)
      ti_is_optimization_in_list(__is_from_baseline ${OPT} ${CPU_BASELINE})
    endif()
  endif()
  ti_is_optimization_in_list(__is_from_dispatch ${OPT} ${CPU_DISPATCH_REQUIRE})
  if(NOT __is_from_dispatch)
    ti_is_optimization_in_list(__is_from_dispatch ${OPT} ${CPU_DISPATCH})
  endif()
  if(__is_from_dispatch OR __is_from_baseline OR CPU_BASELINE_DETECT)
    ti_check_compiler_optimization(${OPT})
  endif()
  if(CPU_BASELINE_DETECT AND NOT __is_from_baseline AND NOT __is_disabled)
    ti_is_optimization_in_list(__is_from_baseline ${OPT} ${CPU_BASELINE_FINAL})
  endif()
  if(CPU_${OPT}_SUPPORTED)
    ...
    elseif(__is_from_baseline AND NOT __is_disabled)
      ...
      if(NOT CPU_${OPT}_ENABLED_DEFAULT)  # Don't change compiler flags in 'detection' mode
        ti_append_optimization_flag(CPU_BASELINE_FLAGS ${OPT})
      endif()
    endif()
  endif()
endforeach()
...
macro(ti_compiler_optimization_options)
  set(__flags "${TIGER_EXTRA_CXX_FLAGS} ${CPU_BASELINE_FLAGS}")
  if(NOT __flags STREQUAL CACHED_CPU_BASELINE_FLAGS)
    set(CACHED_CPU_BASELINE_FLAGS "${__flags}" CACHE INTERNAL "" FORCE)
    ti_clear_vars(HAVE_CPU_BASELINE_FLAGS)
  endif()
  ti_check_compiler_flag(CXX "${__flags}" HAVE_CPU_BASELINE_FLAGS)
  ...
  foreach(OPT ${CPU_DISPATCH_FINAL})
    ...
    foreach(OPT2 ${CPU_KNOWN_OPTIMIZATIONS})
      ...
      ti_is_optimization_in_list(__is_from_baseline ${OPT2} ${CPU_BASELINE_FINAL})
      if(NOT __is_from_baseline)
        ti_is_optimization_in_list(__is_active ${OPT2} ${OPT})
        if(__is_active)
          ti_append_optimization_flag(__dispatch_flags ${OPT2})
          list(APPEND __dispatch_definitions "TI_CPU_COMPILE_${OPT2}=1")
          list(APPEND __dispatch_opts "${OPT2}")
        endif()
        ti_is_optimization_in_force_list(__is_force ${OPT2} ${OPT})
        if(__is_force)
    ...
    endforeach()
    set(__flags "${TIGER_EXTRA_CXX_FLAGS} ${__dispatch_flags}")
    if(NOT __flags STREQUAL CACHED_CPU_DISPATCH_${OPT}_FLAGS)
      set(CACHED_CPU_DISPATCH_${OPT}_FLAGS "${__flags}" CACHE INTERNAL "" FORCE)
      ti_clear_vars(HAVE_CPU_DISPATCH_FLAGS_${OPT})
    endif()
    ti_check_compiler_flag(CXX "${__flags}" HAVE_CPU_DISPATCH_FLAGS_${OPT})
...
macro(ti_compiler_optimization_options_finalize)
  ...
  if(MSVC)
    ...
    set(TIGER_EXTRA_FLAGS "${TIGER_EXTRA_FLAGS} /Oi")
  endif(MSVC)
endmacro()

macro(ti_compiler_optimization_process_sources SOURCES_VAR_NAME LIBS_VAR_NAME TARGET_BASE_NAME)
  set(__result "")
  set(__result_libs "")
  foreach(OPT ${CPU_DISPATCH_FINAL})
    set(__result_${OPT} "")
  ...
  foreach(fname ${${SOURCES_VAR_NAME}})
    ...
    if(fname_LOWER MATCHES ".+\\.([^\\.]*)\\.cpp$")
      ...
      if(OPT_ MATCHES "(CUDA.*|DISPATCH.*|OCL)") # don't touch files like filename.cuda.cpp
        ...
        #continue()
      elseif(TI_DISABLE_OPTIMIZATION OR NOT TI_ENABLE_INTRINSICS)
        ti_get_smart_file_name(fname_ "${fname}")
        ...
        if(__definitions)
          list(APPEND __definitions "TI_CPU_DISPATCH_MODE=${OPT_}")
        else()
          set(__definitions "TI_CPU_DISPATCH_MODE=${OPT_}")
        endif()
...
#message("${OPT}: ${__result_${OPT}}")
      if(CMAKE_GENERATOR MATCHES "^Visual"
          OR TIGER_CMAKE_CPU_OPTIMIZATIONS_FORCE_TARGETS
      )
        ...
        ti_append_dependant_targets(${TARGET_BASE_NAME} ${TARGET_BASE_NAME}_${OPT})
...
macro(ti_compiler_optimization_fill_cpu_config)
  set(TIGER_CPU_BASELINE_DEFINITIONS_CONFIGMAKE "")
  foreach(OPT ${CPU_BASELINE_FINAL})
    set(TIGER_CPU_BASELINE_DEFINITIONS_CONFIGMAKE "${TIGER_CPU_BASELINE_DEFINITIONS_CONFIGMAKE}
#define TI_CPU_COMPILE_${OPT} 1
#define TI_CPU_BASELINE_COMPILE_${OPT} 1
")
  endforeach()

  set(TIGER_CPU_BASELINE_DEFINITIONS_CONFIGMAKE "${TIGER_CPU_BASELINE_DEFINITIONS_CONFIGMAKE}
#define TI_CPU_BASELINE_FEATURES 0 \\")
  foreach(OPT ${CPU_BASELINE_FINAL})
  ...
  foreach(OPT ${__dispatch_modes})
    set(TIGER_CPU_DISPATCH_DEFINITIONS_CONFIGMAKE "${TIGER_CPU_DISPATCH_DEFINITIONS_CONFIGMAKE}
#define TI_CPU_DISPATCH_COMPILE_${OPT} 1")
  endforeach()

  set(TIGER_CPU_DISPATCH_DEFINITIONS_CONFIGMAKE "${TIGER_CPU_DISPATCH_DEFINITIONS_CONFIGMAKE}
\n\n#define TI_CPU_DISPATCH_FEATURES 0 \\")
  foreach(OPT ${__dispatch_modes})
    if(NOT DEFINED CPU_${OPT}_FEATURE_ALIAS OR NOT "x${CPU_${OPT}_FEATURE_ALIAS}" STREQUAL "x")
      set(TIGER_CPU_DISPATCH_DEFINITIONS_CONFIGMAKE "${TIGER_CPU_DISPATCH_DEFINITIONS_CONFIGMAKE}
    , TI_CPU_${OPT} \\")
  ...
  set(TIGER_CPU_CONTROL_DEFINITIONS_CONFIGMAKE "// AUTOGENERATED, DO NOT EDIT\n")
  foreach(OPT ${CPU_ALL_OPTIMIZATIONS})
    if(NOT DEFINED CPU_${OPT}_FEATURE_ALIAS OR NOT "x${CPU_${OPT}_FEATURE_ALIAS}" STREQUAL "x")
      set(TIGER_CPU_CONTROL_DEFINITIONS_CONFIGMAKE "${TIGER_CPU_CONTROL_DEFINITIONS_CONFIGMAKE}
#if !defined TI_DISABLE_OPTIMIZATION && defined TI_ENABLE_INTRINSICS && defined TI_CPU_COMPILE_${OPT}
#  define TI_TRY_${OPT} 1
#  define TI_CPU_FORCE_${OPT} 1
#  define TI_CPU_HAS_SUPPORT_${OPT} 1
#  define TI_CPU_CALL_${OPT}(fn, args) return (cpu_baseline::fn args)
#  define TI_CPU_CALL_${OPT}_(fn, args) return (opt_${OPT}::fn args)
#elif !defined TI_DISABLE_OPTIMIZATION && defined TI_ENABLE_INTRINSICS && defined TI_CPU_DISPATCH_COMPILE_${OPT}
#  define TI_TRY_${OPT} 1
#  define TI_CPU_FORCE_${OPT} 0
#  define TI_CPU_HAS_SUPPORT_${OPT} (ti::checkHardwareSupport(TI_CPU_${OPT}))
#  define TI_CPU_CALL_${OPT}(fn, args) if (TI_CPU_HAS_SUPPORT_${OPT}) return (opt_${OPT}::fn args)
#  define TI_CPU_CALL_${OPT}_(fn, args) if (TI_CPU_HAS_SUPPORT_${OPT}) return (opt_${OPT}::fn args)
#else
#  define TI_TRY_${OPT} 0
#  define TI_CPU_FORCE_${OPT} 0
#  define TI_CPU_HAS_SUPPORT_${OPT} 0
#  define TI_CPU_CALL_${OPT}(fn, args)
#  define TI_CPU_CALL_${OPT}_(fn, args)
#endif
#define __TI_CPU_DISPATCH_CHAIN_${OPT}(fn, args, mode, ...)  TI_CPU_CALL_${OPT}(fn, args); __TI_EXPAND(__TI_CPU_DISPATCH_CHAIN_ ## mode(fn, args, __VA_ARGS__))
")
    endif()
  endforeach()
...
")
  ...
  set(__file "${${PROJECT_NAME}_SOURCE_DIR}/modules/base/include/${TI_INTERNAL_NAME}/base/ti_cpu_helper.h")
  if(EXISTS "${__file}")
    file(READ "${__file}" __content)
  endif()
  if(__content STREQUAL TIGER_CPU_CONTROL_DEFINITIONS_CONFIGMAKE)
    #message(STATUS "${__file} contains same content")
  else()
    file(WRITE "${__file}" "${TIGER_CPU_CONTROL_DEFINITIONS_CONFIGMAKE}")
    message(WARNING "${__file} is updated")
  endif()
endmacro()

macro(__ti_add_dispatched_file filename target_src_var src_directory dst_directory precomp_hpp optimizations_var)
  if(NOT TIGER_INITIAL_PASS)
    set(__codestr "
#include \"${src_directory}/${precomp_hpp}\"
#include \"${src_directory}/${filename}.simd.hpp\"
")

    set(__declarations_str "#define TI_CPU_SIMD_FILENAME \"${src_directory}/${filename}.simd.hpp\"")
    set(__dispatch_modes "BASELINE")

    set(__optimizations "${${optimizations_var}}")
    if(TI_DISABLE_OPTIMIZATION OR NOT TI_ENABLE_INTRINSICS)
      set(__optimizations "")
    endif()
...
")
    ...
    set(__declarations_str "${__declarations_str}
#define TI_CPU_DISPATCH_MODES_ALL ${__dispatch_modes}

#undef TI_CPU_SIMD_FILENAME
")
...
macro(ti_add_dispatched_file filename)
  set(__optimizations "${ARGN}")
  if(" ${ARGV1}" STREQUAL " TEST")
    list(REMOVE_AT __optimizations 0)
    __ti_add_dispatched_file("${filename}" "TIGER_MODULE_${the_module}_TEST_SOURCES_DISPATCHED" "${CMAKE_CURRENT_LIST_DIR}/test" "test/" "test_precomp.hpp" __optimizations)
  else()
    __ti_add_dispatched_file("${filename}" "TIGER_MODULE_${the_module}_SOURCES_DISPATCHED" "${CMAKE_CURRENT_LIST_DIR}/src" "" "precomp.hpp" __optimizations)
  endif()
endmacro()


# Workaround to support code which always require all code paths
macro(ti_add_dispatched_file_force_all)
  set(__CPU_DISPATCH_INCLUDE_ALL 1)
  ti_add_dispatched_file(${ARGN})
