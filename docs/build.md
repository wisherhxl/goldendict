...
Visual Studio 2026 can build the current `compiler.version=194` Conan profile
with its optional MSVC 14.44 toolset. Select that toolset with
`VsDevCmd.bat -vcvars_ver=14.44` and add both
`-c tools.microsoft.msbuild:vs_version=18` and
`-c tools.cmake.cmaketoolchain:generator=Ninja` to `conan install`. The Ninja
generator is required because the repository's CMake 3.26 tool does not know
the Visual Studio 18 project generator; Ninja still invokes the selected VS
2026 compiler. The selected compiler binaries and standard-library headers
must come from the same MSVC toolset.

Before the first Windows dependency install, set `CONAN_HOME` to a short
absolute path, for example `D:\c2`. Qt WebEngine generates Chromium source
paths that can otherwise exceed MSVC's path limit while Qt is built in the
default user-profile Conan cache. Keep the same `CONAN_HOME` value for later
Conan commands that consume the resulting packages.
...
```sh
python conan/recipes/qt-multimedia-backend/export_recipe.py
...
python conan/recipes/qt-multimedia-backend/export_recipe.py
conan export conan/recipes/opencc
conan install . --build=missing -s build_type=Release -o "qt/*:shared=True"
build\generators\conanbuild.bat
build\generators\conanrun.bat
cmake --preset conan-default
...
Qt Multimedia's FFmpeg and Linux PulseAudio integrations require the focused
recipe adapter generated by
`conan/recipes/qt-multimedia-backend/export_recipe.py`. Run the script before
dependency resolution on every platform. The GoldenDict graph enables FFmpeg
everywhere, enables PulseAudio only on Linux, and leaves the Windows build free
of Linux-only PulseAudio, ALSA, libcap, and Xorg dependencies.
