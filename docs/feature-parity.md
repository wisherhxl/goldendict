...
| Dictionary info/headword browser | slice | Phase 8 | facade-backed catalog identity, redacted source, language, count, and selectable plain-text description display for metadata-bearing migrated formats; copy-source/open-containing-folder actions, bounded per-dictionary prefix, wildcard, and regular-expression filtering, lookup activation, atomic displayed-headword export, and fixture-backed UI smokes; R3a.1/R3a.2 provide authenticated bounded full-list enumeration for StarDict and 11 article formats, while R3a.3 formats and R3b file export remain excluded |
