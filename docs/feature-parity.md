...
| Full-text search | slice | Phase 5/6/8 | Installed bounded query and all twelve private adapters are accepted; P8-FT-1 through P8-FT-13 complete the cancellable request path, persisted query modes and bounds, capability/filter projection, modeless dialog, response model/synchronization, one visible ordered result list, and private Widgets-owned activation intent. P8-FT-14 completes the Core-owned scoped lookup-navigation prerequisite: navigation retains absent, authoritative-empty, and ordered non-empty dictionary scope through identity, history, session persistence/restoration, MainWindow replay, and participation refresh without widening. Ordinary unscoped lookup and suggestion projection remain unchanged. The approved later activation target is the result headword across that complete scope, not only `result.dictionary.id`; signal connection, tab/query/history behavior, exact `document_id` selection, and highlighting remain separate. Selection/focus/retention; decoration; counts and empty/error/partial presentation; Preferences/index policy; index lifecycle; adapters, index formats and legacy `_FTS`; dependencies; build-system work; and unrelated behavior remain separate. Evidence is migrated `desktop_facade.h`, Core tab/session tests, `main_window.cpp` lookup/navigation/filter construction, P8-FT-7 projection, and pinned legacy `fulltextsearch.cc:594-610` plus `mainwindow.cc:3002-3014`. No successor after P8-FT-14 is selected or ranked. |
