...
| ABBYY Lingvo DSL | slice | text batch | bounded plain/compressed decoding up to 1 GiB, including the approved 595,963,560-byte decoded Britannica corpus member; directives; Qt 5-compatible optional, alternate, unsorted-zone, escaped, repeated, `~`, and `^~` headword preprocessing with per-line expansion bounds; common markup/links; adjacent plain/gzip `_abrv` companions with non-standalone ownership, expanded keys, markup-stripped values, `dsl_p` tooltips, and fail-open optional parsing; confined directory resources; and adjacent `.dsl.files.zip`/`.dsl.dz.files.zip` resources through bounded central-directory indexing, classic wrapped-count and ZIP64 compatibility, lazy stored/deflated member reads, CRC verification, and source-change rejection. All 16 approved real-corpus DSL dictionaries match the frozen Qt 5 article and headword counts with zero diagnostics, all five R3.4 archive-resource probes match exact size and SHA-256, and all three cataloged abbreviation components match ownership in clean and warm states; P6-FT-7 private full-text support is accepted with per-article deduplication, first-expanded-record provenance, inert text, and sole `.dsl`/`.dsl.dz` revision; nested cards remain |
