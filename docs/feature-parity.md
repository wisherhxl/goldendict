...
| Main tabs and navigation | slice | Phase 8 | bounded transport-neutral lifecycle, complete core session DTO export/atomic restore and canonical current-config storage, plus visible retained WebEngine tabs, foreground/background opening, deterministic close/middle-close/close-others fallback, synchronized per-tab query/group/title state, and facade-backed internal-link back/forward navigation; application restart wiring, legacy migration, placement, and opening preferences remain |
