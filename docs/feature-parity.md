...
| Dictionary info/headword browser | slice | Phase 8 | facade-backed catalog identity/source display, backend-owned article/headword counts for all migrated local formats, plain-text descriptions for metadata-bearing Aard/MDict/SLOB/ZIM dictionaries, bounded per-dictionary prefix suggestions, lookup activation, atomic displayed-headword export, and fixture-backed UI smokes; remaining format descriptions, unbounded full-list export, advanced filtering, and local-file actions remain |
