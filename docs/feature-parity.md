...
| Full-text search | slice | Phase 5/6/8 | Installed bounded query and all twelve private adapters are accepted; P8-FT-1 through P8-FT-13 complete the cancellable request path, persisted query modes and bounds, capability/filter projection, modeless dialog, response model/synchronization, visible ordered result list, and private activation intent. P8-FT-14 preserves absent, authoritative-empty, and ordered nonempty lookup scope through navigation identity, history, session restoration, replay, and participation refresh. P8-FT-15 privately binds the exact submitted scope to the accepted response and delivers result and immutable scope by value. P8-FT-16 is the sole selected smallest dependency-ready leaf: MainWindow privately connects activation to existing scoped current-tab navigation, using the exact headword for query/title, activating no new/background tab, leaving the main query edit unchanged, capturing the active group, and copying ordered or authoritative-empty accepted scope unchanged. Only successful `OpenArticleTab` proceeds to tab synchronization, session mutation, and `StartNavigationLookup(..., true)`; missing facade/dialog/context, invalid activation/navigation, or tab-limit failure causes no lookup/history/session mutation and retains existing failure behavior. Exact `document_id`, source targeting, match/excerpt metadata, highlighting/ignore-diacritics/WebEngine; selection/focus/retention; decoration; counts and response states; Preferences/index policy; index lifecycle; adapters/index formats and legacy `_FTS`; persistence beyond navigation; dependencies/build work; and unrelated parity remain separately decomposed and unranked. Evidence is migrated dialog activation/context code and tests, P8-FT-7, P8-FT-13 through P8-FT-15, P8-FT-14 Core navigation/session coverage, MainWindow conventions, and pinned legacy `fulltextsearch.cc:594-610` plus `mainwindow.cc:3002-3014`. No successor after P8-FT-16 is selected or ranked. |
