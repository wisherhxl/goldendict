...
| Full-text search | slice | Phase 5/6/8 | Installed bounded query and all twelve private adapters are accepted; P8-FT-1 through P8-FT-13 complete the cancellable request path, persisted query modes and bounds, capability/filter projection, modeless dialog, response model/synchronization, visible ordered result list, and private activation intent. P8-FT-14 preserves absent, authoritative-empty, and ordered nonempty lookup scope through navigation identity, history, session restoration, replay, and participation refresh. The post-P8-FT-14 audit selects only P8-FT-15: privately snapshot the submitted scope, associate it with only the accepted response generation, and deliver it by value with activation. Direct MainWindow connection is not yet dependency-ready because mutable projected scope is not response identity. The locked later behavior uses the exact result headword in the current activated tab, leaves the main query edit unchanged, constructs `kLookup` navigation with the active group and complete accepted scope, records existing tab/lookup history only after successful navigation, and preserves authoritative-empty without fallback. Exact `document_id`, source targeting, match/excerpt metadata, highlighting/ignore-diacritics/WebEngine; selection/focus/retention; decoration; counts and response states; Preferences/index policy; index lifecycle; adapters/index formats and legacy `_FTS`; persistence beyond navigation; dependencies/build work; and unrelated parity remain separate. Evidence is migrated dialog lifecycle/tests, P8-FT-7, P8-FT-13, P8-FT-14, MainWindow conventions, and pinned legacy `fulltextsearch.cc:594-610` plus `mainwindow.cc:3002-3014`. No successor after P8-FT-15 is selected or ranked. |
