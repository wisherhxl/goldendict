...
| Full-text search | slice | Phase 5/6/8 | installed bounded query and private reference-index contract complete; all twelve private format adapters are accepted through P6-FT-14 EPWING; P8-FT-1 completes the private cancellable asynchronous Widgets request controller; P8-FT-2 completes four-mode preference persistence through additive public `PlainText = 3`, preserving migrated `0/1/2` and explicitly translating legacy `0/1/2/3`; P8-FT-3 completes the independently validated per-dictionary article cap; the post-P8-FT-3 audit selects only P8-FT-4, the authorized installed DTO/validation/aggregator prerequisite that makes the per-dictionary limit optional with an engaged `100` default, preserves global default `20`, widens the independent global bound to `1000000`, and reserves fixed later-dialog cap `100000`; dialog/query controls, dictionary selection, results/activation, highlighting, Preferences policy, index visibility, and background lifecycle remain separate and unselected; no successor after P8-FT-4 is selected or ranked |
