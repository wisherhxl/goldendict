...
| History | slice | Phase 8 | bounded UTF-8/group-aware current store, legacy line-format migration, current-state precedence, atomic persistence, application lookup recording, case-insensitive live filtering, clear action, bounded UTF-8 text import, atomic text export, and a reusable history pane; group-selection UI remains |
...
| Dictionary info/headword browser | slice | Phase 8 | facade-backed catalog identity/source display, plain-text descriptions for metadata-bearing Aard/MDict/SLOB/ZIM dictionaries, bounded per-dictionary prefix suggestions, lookup activation, atomic displayed-headword export, and fixture-backed UI smokes; counts, remaining format descriptions, unbounded full-list export, advanced filtering, and local-file actions remain |
