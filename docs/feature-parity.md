...
| Dictionary info/headword browser | slice | Phase 8 | facade-backed catalog identity/source display, backend-owned article/headword counts for all migrated local formats, plain-text descriptions for metadata-bearing Aard/MDict/SLOB/ZIM dictionaries, copy-source/open-containing-folder actions, bounded per-dictionary prefix, wildcard, and regular-expression filtering, lookup activation, atomic displayed-headword export, and fixture-backed UI smokes; remaining format descriptions and unbounded full-list export remain |
