...
| Full-text search | slice | Phase 5/6/8 | installed bounded query and private reference-index contract complete; all twelve private format adapters are accepted through P6-FT-14 EPWING; P8-FT-1 completes the private cancellable asynchronous Widgets request controller; P8-FT-2 completes four-mode preference persistence through additive public `PlainText = 3`, preserving migrated `0/1/2` and explicitly translating legacy `0/1/2/3`; P8-FT-3 completes the independently validated per-dictionary article cap; P8-FT-4 completes the optional installed per-dictionary limit and widened independent global bound; P8-FT-5 completes the private non-integrated Widgets composer mapping all query controls, checked optionals, fixed global cap `100000`, existing timeout, and unfiltered dictionary state into `FullTextQuery` without submission or persistence mutation; P8-FT-6 completes additive installed `DictionaryIdentity::supports_full_text_search`, whose Core-owned mapping is true exactly for the twelve private full-text backends and false for unsupported/resource/remote backends, with no persistence or index-readiness meaning; P8-FT-7 completes the private non-submitting Widgets projection of public capability, ordered selected-group membership, configured muting, and visible ephemeral dictionary-bar checks into an always-active ordered query filter, including an active empty selection; P8-FT-8 completes the private modeless dialog shell and MainWindow/Search-action integration, with exact action mappings, singleton modeless lifetime, current lookup-text initialization, current-state reprojection, safe controller/service teardown, and no submission or persistence; P8-FT-9 completes private request submission, cancellation, generation-based replacement, stale-completion suppression, unchanged terminal-response retention, progress/idle state, and safe teardown; P8-FT-10 completes the private non-integrated per-document response projection model, preserving Core order, duplicate headwords, and all result metadata while retaining no error/partial state and defining no dialog attachment or presentation; P8-FT-11 completes private synchronization of the dialog's retained current response with its child response model, including replacement clearing, generation-current projection, unchanged response-level errors/partial state, stale/cancelled suppression, and no visible result view; P8-FT-12 is selected as the private attachment of one visible `QListView` to that model, exposing exact ordered UTF-8 rows without decoration, selection policy, state presentation, or activation. Dictionary decoration/extra roles, selection/focus/retention, counts and empty/error/partial presentation, activation/navigation/lookup, highlighting/WebEngine handoff, Preferences enablement/index policy, and index readiness/visibility/status/background lifecycle remain separately decomposed and unselected; public APIs/DTOs, persistence, adapters, `.gdfts`, legacy `_FTS`, dependencies, and build-system changes are excluded. Evidence is migrated `full_text_search_dialog.h/.cpp`, `full_text_response_model.h/.cpp`, and focused tests plus pinned legacy `fulltextsearch.hh:135-156`, `fulltextsearch.cc:518-610,685-750`, and `fulltextsearch.ui:99-238`. No successor after P8-FT-12 is selected or ranked. |
