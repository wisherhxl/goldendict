...
| Preferences and source editor | slice | Phase 8 | core-owned bounded defaults and portable migration plus one bounded ordered editor for local directories, MediaWiki, website, Forvo, DICT, and shell-free external-program sources with atomic validation, save, and facade recomposition; the first legacy-shaped Preferences leaf exposes only the already-backed new-tab placement and activation controls through the same complete-candidate transaction; credentials, recursive-path/icon fields, and the remaining runtime-unbacked Preferences groups remain |
