...
| --- | --- | --- | --- |
| StarDict | slice | vertical slice | discover/index/search/article/resource accepted; P6-FT-1 full-text adapter accepted with primary-only ingestion, assembled plain text, private lifecycle, and installed-service coverage; R3.6 Units 1-3 accept the pinned companion matrix, retained 32-bit index and declared counts, exact known/unknown language fallback, bounded classic Qt metadata conversion, HTML-headwords, synonym ownership/filtering, deterministic lookup/enumeration, cancellation and malformed/truncated/range semantics, plus cancellation-aware ordered same-sequence/per-record decoding for every Qt 5 article field type with safe XDXF attribution/external references, visual-format line/indent conversion, ordered `src`/`losrc`/`hisrc` images, empty-reference fallback, non-converted `rref start` presentation, first-strong per-line direction including `sd`/`ug`, HTML audio, frozen single-pass nested PowerWord behavior, extended Pango presentation, the complete legacy image/audio extension set, link, media, and source-headword presentation; private generated and full-text caches rebuild for the current semantics; resource-ZIP and icon-evidence Unit 4 remains |
