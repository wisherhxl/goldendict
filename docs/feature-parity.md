...
| --- | --- | --- | --- |
| StarDict | slice | vertical slice | discover/index/search/article/resource accepted; P6-FT-1 full-text adapter accepted with primary-only ingestion, assembled plain text, private lifecycle, and installed-service coverage; R3.6 Units 1-2 accept the pinned companion matrix plus retained 32-bit index, declared count, exact known/unknown language fallback, bounded classic Qt metadata entities/plain text, HTML-headword, synonym ownership/filtering, deterministic lookup/enumeration, cancellation, and malformed/truncated/range semantics; private generated and full-text caches rebuild for the current semantics; article-field, resource-ZIP, and icon-evidence Units 3-4 remain |
