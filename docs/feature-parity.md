...
| Morphology and transliteration | later | Phase 5 | the private bounded Russian, German, and Greek table primitives preserve pinned longest-match, case-sensitive, and unmatched-Unicode behavior with data-driven tests; German also preserves the intentionally disabled reverse mappings, while Greek covers modern, classical Beta Code, alternate diacritic order, and tonos/oxia conversion; dictionary-provider composition, configuration, morphology, and other transliterations remain later per-language leaves |
