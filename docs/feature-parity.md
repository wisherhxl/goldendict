...
The independent post-P8-FT-36 audit selects only P8-FT-37: preserve the
private Search button's legacy-authenticated explicit-default and
non-auto-default policy. Pinned legacy `fulltextsearch.ui:204-214` sets
`default == true` and `autoDefault == false`; current Widgets sets only the
explicit default, and existing focused coverage pins neither property. The
leaf is limited to `fullTextSearchButton`, its existing focused dialog test,
and stability across construction and request-state transitions. Tab order,
focus transfer, broader keyboard dispatch, other buttons, public/Core or
installed contracts, and all unrelated parity remain excluded and unranked.
The Release registration baseline remains exactly 109 tests, and installed C
and C++ consumers remain unchanged and source-compatible. No successor after
P8-FT-37 is selected or ranked.
