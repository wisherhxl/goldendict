...
| Legacy configuration migration | slice | Phase 8 | bounded XML import of dictionary and sound-directory paths, ordered dictionary groups and stable metadata, plus portable non-secret P1 preferences with strict conversions and default fallback; current-config precedence, atomic new-format persistence, and untouched legacy source fixtures are pinned; sources, credentials, layout/session state, and platform-specific legacy location discovery remain |
...
| Main tabs and navigation | slice | Phase 8 | bounded transport-neutral lifecycle, complete core session DTO export/atomic restore and canonical current-config storage, application restore/save wiring with two-start coverage, plus visible retained WebEngine tabs, foreground/background opening, deterministic close/middle-close/close-others fallback, synchronized per-tab query/group/title state, and facade-backed internal-link back/forward navigation; legacy migration, placement, and opening preferences remain |
...
| Preferences and source editor | slice | Phase 8 | core-owned bounded current preference defaults, validation, deterministic atomic round-trip, older-current-format compatibility, and bounded portable non-secret legacy preference migration; source editing, credentials, Widgets UI, and behavior wiring remain |
