...
| Full-text search | slice | Phase 5/6/8 | installed bounded query and private reference-index contract complete; all twelve private format adapters are accepted through P6-FT-14 EPWING; P8-FT-1 completes the private cancellable asynchronous Widgets request controller; P8-FT-2 selects only four-mode preference persistence through additive public `PlainText = 3`, preserving migrated `0/1/2` and explicitly translating legacy `0/1/2/3`; dialog/query controls, dictionary selection, results/activation, highlighting, Preferences policy, index visibility, and background lifecycle remain separate and unselected; no successor is selected or ranked |
