...
| Preferences and source editor | slice | Phase 8 | core-owned bounded defaults and portable migration plus one bounded ordered editor for local directories, MediaWiki, website, Forvo, DICT, and shell-free external-program sources with atomic validation, save, and facade recomposition; legacy-shaped General controls expose backed tab placement/activation, bounded history recording/size, favorites deletion confirmation, article collapsing, default-off expandable DSL optional parts, Unicode-scalar input-phrase rejection, capability-aware exact diacritic matching, and default-on exact synonym-to-primary expansion for StarDict, Babylon BGL, and GLS through the same complete-candidate transaction; suggestions and unsupported sources remain unchanged, while the favorites save interval and remaining runtime-unbacked Preferences groups remain absent |
