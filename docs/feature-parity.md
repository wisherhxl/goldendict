...
| EPWING | slice | specialized batch | bounded `CATALOGS`/subbook/index parsing, Latin-1 and JIS X 0208 text, folded lookup/suggestions, internal references, and confined subbook resources; P6-FT-13 is selected to expose headword-independent `(text-file-ordinal, page, offset)` ownership and a complete ordered `CATALOGS`/optional `LANGUAGE`/selected-subbook-tree revision; EPWING remains typed unsupported and no adapter or successor is selected or ranked; compressed HONMON, mixed GB2312, gaiji/media rendering, grouped indexes, and full-text parity remain |
