...
| Full-text search | slice | Phase 5/6/8 | installed bounded query and private reference-index contract complete; P6-FT-1 StarDict through P6-FT-10 ZIM are accepted; P6-FT-11 closes MDict ownership/revision and P6-FT-12 accepts the private MDict adapter as the eleventh format; P6-FT-13 completes EPWING ownership/revision and the post-prerequisite audit selects P6-FT-14, the private EPWING adapter, as the twelfth format; no successor is selected or ranked |
...
| EPWING | slice | specialized batch | bounded `CATALOGS`/subbook/index parsing, Latin-1 and JIS X 0208 text, folded lookup/suggestions, internal references, and confined subbook resources; P6-FT-13 accepts headword-independent `(text-file-ordinal, page, offset)` ownership and the complete ordered source revision; P6-FT-14 is selected for one document per retained physical article, first-record ownership, exact five-component provenance, inert assembled text, complete lifecycle, and twelfth-format dispatch; implementation remains gated and no successor is selected or ranked; compressed HONMON, mixed GB2312, gaiji/media rendering, grouped indexes, and later parity remain |
