...
| --- | --- | --- | --- |
| StarDict | slice | vertical slice | discover/index/search/article/resource accepted; P6-FT-1 full-text adapter accepted with primary-only ingestion, assembled plain text, private lifecycle, and installed-service coverage; R3.6 Unit 1 accepts the pinned plain/compressed companion name, case, and precedence matrix, recoverable optional-synonym behavior, bounded typed decompression failures, exact consumed-source identity, and `stardict-records-v3` cache rebuild semantics; index/metadata, article-field, resource-ZIP, and icon-evidence Units 2-4 remain |
