...
| Full-text search | slice | Phase 5/6/8 | Installed bounded query and all twelve private adapters are accepted; P8-FT-1 through P8-FT-12 complete the cancellable request path, persisted query modes and bounds, capability/filter projection, modeless dialog, response model/synchronization, and one visible ordered result list. The post-P8-FT-12 audit selects exactly one leaf, P8-FT-13: private Widgets-owned activation intent carrying a safe snapshot of the existing `FullTextResult` exactly once for a valid single primary-button click or current-row Return/Enter, with no duplicate double-click activation and no invalid/reset-index delivery. Selection/focus/retention; dictionary decoration/extra roles; counts and empty/error/partial presentation; MainWindow lookup, exact dictionary scoping, tab/history/navigation, and highlighting/WebEngine handoff; Preferences enablement/index policy; and index readiness/visibility/status/background lifecycle remain separately decomposed. Public APIs/DTOs, persistence, adapters, `.gdfts`, legacy `_FTS`, dependencies, and build-system changes are excluded. Evidence is migrated `full_text_search_dialog.h/.cpp`, `full_text_response_model.h/.cpp`, and focused tests plus pinned legacy `fulltextsearch.cc:292-293,594-610,664-673` and `fulltextsearch.hh:227,232-233`. No successor after P8-FT-13 is selected or ranked. |
