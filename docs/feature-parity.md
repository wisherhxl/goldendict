...
| Dictionary identity and index lifecycle | slice | Phase 4 | deterministic index tests plus paired changed-source, unavailable-companion, stale-index cleanup, bounded full-text cancellation/progress, and recovery evidence with matching observer-visible diagnostics |
...
| Morphology and transliteration | later | Phase 5 | the private bounded Russian, German, Greek, three-variant Belarusian, and two-variant Hepburn Romaji table primitives preserve pinned longest-match and unmatched-Unicode behavior with data-driven tests; Russian, German, and Greek retain case-sensitive mappings, Belarusian retains simple case folding, first-wins duplicates, and exact 334/333, 535/534, and 446/436 declaration/effective counts, and Hepburn retains the same folding/duplicate rules with exact Hiragana 139/139 and Katakana 167/165 counts plus the first `va`/`vo` targets; German also preserves disabled reverse mappings and Greek covers modern/classical Beta Code, alternate diacritic order, and tonos/oxia conversion. Private bounded Hunspell discovery preserves the pinned non-recursive `.aff`/`.AFF` plus `.dic`/`.DIC` companion and basename-identity rules. The private content loader preserves those original bytes and pair ordering while strictly validating the `SET` encoding, complete affix/dictionary content, entry-header syntax, independently bounded actual entry counts, filesystem safety, and explicit resource bounds through typed failures; stale advisory header counts remain accepted like Qt 5 and Hunspell. Private Conan Hunspell 1.7.2 composition consumes that loader and preserves bounded serialized exact membership for UTF-8 and legacy-encoded original dictionaries through the existing backend contract. Its private `SynonymBackend` capability preserves bounded legacy-compatible single-word `analyze()` stems, including declared-encoding conversion, outer trimming, the 80-character limit, `st:`-only extraction, comment stripping, engine order and duplicates, simple-case-equivalent filtering, and request cancellation/deadlines. Whitespace-containing expressions use the pinned private compound path: at most 21 lexical/separator runs preserve exact punctuation and Unicode whitespace while declared-encoding serialized analysis supplies the original plus first two stems per word for result-limit-bounded legacy-order reconstruction, with the unchanged expression removed and request controls observed. A separate private `CompoundAlternateWritingsBackend` now returns nothing for single words and delegates original whitespace-containing input to that same bounded reconstruction without changing lookup orchestration. Its private `LookupPrefix()` restores pinned legacy whole-query membership rather than enumeration: bounded trimmed single-word queries are converted to the declared encoding, checked by serialized `spell()`, and return at most the trimmed original UTF-8 headword. The private spelling-suggestion article leaf replaces the exact-membership placeholder with pinned bounded `getArticle` semantics: strictly validated and outer-trimmed single-word misspellings call serialized `spell()` then `suggest()` in the declared encoding; correct words and suggestion-free misses return nothing; decoded suggestions retain engine order and duplicates; a simple-case-equivalent suggestion suppresses the complete article; and at most 64 suggestions and 64 KiB of escaped inert HTML enter the existing typed-link sanitizer with cancellation/deadline checkpoints. A separate private `TruePrefixBackend` now enumerates valid proper UTF-8 prefixes longest first through bounded serialized `spell()` checks while excluding the complete input. Public prefix suggestions and installed contracts are unchanged. Nihon-shiki/Kunrei-shiki Romaji and Chinese dictionary registration/user configuration remain later leaves |
...
The private Chinese-conversion foundation is accepted for simplified-to-Taiwan,
simplified-to-Hong-Kong, and traditional-to-simplified conversion through the
packaged OpenCC `s2tw.json`, `s2hk.json`, and `t2s.json` configurations. It
preserves the pinned simple-case fold, enforces UTF-8 input/output bounds and
...
| --- | --- | --- | --- |
| StarDict | done | vertical slice | discover/index/search/article/resource accepted; P6-FT-1 full-text adapter accepted with primary-only ingestion, assembled plain text, private lifecycle, and installed-service coverage; R3.6 accepts the pinned companion matrix, retained 32-bit index and declared counts, exact known/unknown language fallback, bounded classic Qt metadata conversion, HTML-headwords, synonym ownership/filtering, deterministic lookup/enumeration, cancellation and malformed/truncated/range semantics, cancellation-aware ordered same-sequence/per-record decoding for every Qt 5 article field type with safe XDXF attribution/external references, visual-format line/indent conversion, ordered `src`/`losrc`/`hisrc` images, empty-reference fallback, non-converted `rref start` presentation, first-strong per-line direction including `sd`/`ug`, HTML audio, frozen single-pass nested PowerWord behavior, extended Pango presentation, the complete legacy image/audio extension set, link, media, and source-headword presentation, plus confined adjacent `res` and bounded stored/deflated `res.zip` resources with frozen candidate/directory precedence, case-folded archive lookup, CRC/source-change enforcement, byte-identical legacy monochrome TIFF-to-BMP conversion, runtime-ID-correct CSS URL rewriting and selector isolation, and clean/warm valid-probe parity; traversal, absolute, and paired Windows reparse escapes retain the approved typed-rejection exception; private generated and full-text caches rebuild for the current semantics; actual frozen-Qt-5 icon decoding proves candidate precedence/fallback and Qt 6 records an explicit unsupported handoff, while cross-format identity payload and UI publication remain R9.1 |
| Dictd | slice | text batch | bounded `.index` + plain/dictzip data, folded lookup, suggestions, and corruption tests; [R3.7a](dictd-companion-loading-parity.md) restores content-based companion decoding with genuine RA fixtures and semantic full-text cache invalidation; [R3.7b](dictd-index-row-recovery-parity.md) restores skipped malformed-row recovery while preserving accepted-row validation; [R3.7c](dictd-title-metadata-parity.md) restores physical-row title ownership, last-extractable and empty titles, exact title bytes, and persisted-name invalidation; [R3.7d](dictd-headword-count-parity.md) tracks reported fourth-column headword counts independently from searchable records; [R3.7e](dictd-ra-header-parity.md) restores leading-RA header admission while retaining ignored fields and the existing gzip route; [R3.7f](dictd-body-layout-parity.md) restores body escaping, indentation, line structure and direction with semantic full-text migration; [R3.7g.1](dictd-full-text-extraction-parity.md) independently extracts frozen pre-href full text with semantic cache invalidation; [R3.7g.2](dictd-inline-display-parity.md) owns coupled phonetic/reference display, scoped styles and EL-01/N3/N4 inert activation; remaining R3.7 variants and global normal-article typography/template parity (R9.8) stay open; P6-FT-4 private full-text support accepted with byte-range deduplication, metadata exclusion, stable full-text provenance, two-source lifecycle, and mixed-service coverage |
...
| ABBYY Lingvo DSL | slice | text batch | bounded plain/compressed decoding up to 1 GiB, including the approved 595,963,560-byte decoded Britannica corpus member; directives; Qt 5-compatible optional, alternate, unsorted-zone, escaped, repeated, `~`, and `^~` headword preprocessing with per-line expansion bounds; common markup/links; bounded tolerant article rendering with source-form displayed headwords, displayed-headword/definition structure, retained source indentation, and safe DSL presentation semantics; adjacent plain/gzip `_abrv` companions with non-standalone ownership, expanded keys, markup-stripped values, `dsl_p` tooltips, and fail-open optional parsing; confined directory resources; and adjacent `.dsl.files.zip`/`.dsl.dz.files.zip` resources through bounded central-directory indexing, classic wrapped-count and ZIP64 compatibility, lazy stored/deflated member reads, CRC verification, and source-change rejection. All 16 approved real-corpus DSL dictionaries match the frozen Qt 5 article and headword counts with zero diagnostics. The final R3.4 clean/warm pair is equivalent for all five visible article texts, exact resource bytes/media types, component ownership, and orphan handling; P6-FT-7 private full-text support is accepted with per-article deduplication, first-expanded-record provenance, inert text, and sole `.dsl`/`.dsl.dz` revision; nested cards remain |
...
| MDict MDX/MDD | slice | binary batch | bounded 1.x/2.x headers and version-specific key/record tables, MDict 2.x key-info encryption (`Encrypted=2`), legacy invalid-title filename fallback, plain/zlib blocks, strict text decoding, styles, redirects, folded lookup/suggestions, sanitized HTML, and companion MDD resources; MDD volumes retain immutable metadata and read resources on demand across record-block and consecutive-volume boundaries with source-change rejection instead of materializing aggregate decoded data; P6-FT-11 accepts terminal ownership and the complete ordered MDX/MDD revision, and P6-FT-12 accepts private full-text support with terminal deduplication, exact five-component provenance, inert text, and complete MDX/MDD lifecycle; R3.3 accepts paired clean/warm exact lookup, visible article text, and byte-identical resource retrieval from the real corpus base MDD, `.1.mdd`, and `.2.mdd`, including safe custom-markup recovery and trailing record-terminator compatibility; R3.5 accepts the repeated real-corpus lookup, suggestion, article, media, and management matrix; other encryption modes, LZO, and R3.13 parity remain |
| Aard | slice | binary batch | bounded 32/64-bit archive indexes, zlib/bzip2/raw articles, metadata, folded lookup/suggestions, redirects, and sanitized article links; P6-FT-6 private full-text support is accepted with unique-article ingestion, first-record ownership, inert assembled text, and a sole-`.aar` source revision. Startup reuses only a validated current artifact; missing, stale, or corrupt work runs through the cancellable serial Core lifecycle after activation, with owned activation in direct service/facade compositions and paired Qt 5 cancellation and recovery evidence. Multi-volume aggregation, icons, and later parity remain |
...
| Inspector/DevTools integration | slice | Phase 7 | real Inspect/F12 entry points, per-article private DevTools lifecycle, Elements/Console and stale-context tests; [Unit 2](article-inspector-geometry.md) adds shared geometry, Qt 5 import, reliable close/exit saving and the legacy initial size under approved IG-01; [IG-02](article-inspector-parity.md#ig-02-approved-inspector-frontend-visual-boundary) accepts only engine-owned frontend appearance; remaining capability, GoldenDict-controlled visual and platform acceptance stay open |
...
| Qt 5-compatible product shell | slice | CRD Batch A | completed slices restore the product icon/resources, 653 by 538 default logical size, the Qt 5 menu and navigation-toolbar action inventory/order/default state, Welcome tab/page, article style, status bar without a visible or interactive non-legacy native size grip, hidden demonstration controls, default left-search/right-stacked-pane arrangement with a 160-logical-pixel resizable right stack and equal Results/Favorites/History allocation, functional routing of the legacy popup-help link, the default toolbar lookup field with original icons, text, palette, and keyboard/click suggestion-popup behavior, the optional docked lookup composition with one authoritative query, synchronized dock-title group selector, persisted View-menu switching, toolbar constraints, and ordered pale-yellow suggestion list, the four Qt 5 custom dock titles including dynamic grouped/ungrouped Search wording and selector visibility plus the unfiltered current/maximum History count, the stable Qt 5 dictionary/article/word catalog summary after facade publication, the single-purpose new-tab corner button without a non-legacy drop-down menu, and the legacy orange article-tab close control backed by the original product resource; focused shell, routing, menu, Core favorites-export, article-tab, suggestion, facade-preparation, and status-presentation tests pass on Windows, while other transient status wording, detailed dialogs, native scan/tray/audio workflows, and state-by-state visual parity remain open |
...
| Configuration and user-state profile upgrade | done | Qt 6 baseline R3.1 | configuration, History, and Favorites are parsed and staged before publication; a pending transaction completes forward after an interrupted publication; malformed input publishes nothing; current files retain precedence; legacy inputs remain unchanged; syntactically valid future configuration records round-trip opaquely; disabled Qt 5 iframe/encoding-marker websites and an empty disabled Forvo language list are preserved; generated failure injection and a disposable genuine Qt 5 profile cover the upgrade path |
| Dictionary info/headword browser | slice | Phase 8 | facade-backed catalog identity, redacted source, language, count, and selectable plain-text description display for metadata-bearing migrated formats; copy-source/open-containing-folder actions, bounded per-dictionary prefix, wildcard, and regular-expression filtering, lookup activation, atomic complete single-dictionary headword export through bounded enumeration, and fixture-backed UI smokes; R3a.1-R3a.3 provide authenticated bounded full-list enumeration for StarDict, 11 article formats, LSA, sound directories, and ZIP sounds, R3b streams that enumeration to a legacy-compatible UTF-8 text file, and the final R3.5 repeated clean/warm pair proves real DSL/MDict information plus two-page cursor behavior and rescan invalidation parity |
