...
| Morphology and transliteration | later | Phase 5 | the private bounded Russian and German table primitives preserve pinned longest-match, case-sensitive, and unmatched-Unicode behavior with data-driven tests; German also preserves the intentionally disabled reverse mappings; dictionary-provider composition, configuration, morphology, and other transliterations remain later per-language leaves |
