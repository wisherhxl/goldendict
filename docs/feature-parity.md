...
| Preferences and source editor | slice | Phase 8 | source editing and the backed General controls for tab placement/activation, hide-single-tab, MRU traversal, ESC hiding with child/modal precedence, history, Favorites deletion, article collapsing, input limits, diacritics, synonyms, and DSL optional parts are accepted through atomic complete-candidate coverage; Preferences as a whole remains open, with P8-PREF-4 article click interaction next and P8-PREF-5 credential-free HTTP CONNECT proxy following it; languages/appearance, tray/autostart, hotkeys/scan, audio, full text, unsupported proxy modes, dictionary-reference context navigation, updates, and cache policy retain explicit Phase 5/6/7/9 prerequisites, including a Phase 7 network-cache ownership audit; delayed history/Favorites saves, WebKit plugins, optional cross-site weakening, unapproved header suppression, Windows scan technologies, and non-dialog legacy fields are intentionally excluded with documented evidence |
