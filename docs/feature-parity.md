...
| Preferences and source editor | slice | Phase 8 | core-owned bounded defaults and portable migration plus one bounded ordered editor for local directories, MediaWiki, website, Forvo, DICT, and shell-free external-program sources with atomic validation, save, and facade recomposition; legacy-shaped General controls expose backed tab placement/activation, bounded history recording/size, and favorites deletion confirmation through the same complete-candidate transaction; the favorites save interval and remaining runtime-unbacked Preferences groups remain absent |
