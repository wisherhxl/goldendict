...
| Full-text search | slice | Phase 5/6/8 | installed bounded query and private reference-index contract complete; P6-FT-1 StarDict support is accepted and P6-FT-2 selects private SDict ingestion before later formats and the Phase 8 workflow |
...
| SDict | slice | text batch | bounded `.dct` parsing, plain/zlib/bzip2 fields, folded search/suggestions, sanitized HTML/link conversion, and corruption tests accepted; P6-FT-2 full-text adapter selected with distinct-offset ingestion, assembled plain text, private lifecycle, and mixed-service coverage pending |
