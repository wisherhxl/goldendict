...
| Initial command-line lookup | slice | Phase 8 | one bounded plain word or case-sensitive legacy `goldendict://[/]...`/`dict://[/]...` operand is normalized privately and handed exactly once to published-facade current-tab navigation; options, ambiguity, malformed or unsupported input are no-ops; single-instance forwarding remains Phase 9 |
