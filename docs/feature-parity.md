...
| Preferences and source editor | slice | Phase 8 | core-owned bounded defaults and portable migration plus one bounded ordered editor for local directories and MediaWiki, website, Forvo, and DICT sources with atomic validation, save, and facade recomposition; external programs, credentials, recursive-path/icon fields, and preferences UI remain |
