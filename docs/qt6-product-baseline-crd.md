...
| --- | --- | --- |
| 2026-09-10 | Use an independent Qt 6 rich-text measurement helper process with its own QGuiApplication/offscreen runtime. Core hosts remain GUI-free and retain collapse policy; no Qt 5 runtime, in-process Core GUI requirement, relaxed resource safety or metric-equivalence waiver is approved. The [focused design](article-richtext-measurement-worker.md) separates converter foundation, bounded client/deployment, later raw-response contract and product hookup. | Runtime direction approved; Unit 1 Ready after documentation delivery, later gates pending |
| 2026-09-10 | Automatic-collapse counting follows frozen Qt 5, including hidden ordinary DSL optional SPAN text. The prior generic hidden-text exclusion is superseded; no visible-only mode or preference is added. The [bounded conformance record](article-optional-collapse-counting.md) separates removing that subtraction from remaining whole-HTML conversion, UTF-16, exact legacy DIV, grouping and exemption gaps. | Approved; bounded verification recorded separately |
