...
CANCELLATION_SCENARIOS = ("cancellation", "cancellation-recovery")
SCENARIO_TOKEN = "__GOLDENDICT_ACCEPTANCE_SCENARIO__"
SUMMARY_SCHEMA = "goldendict-real-dictionary-lifecycle-summary-v1"
RECOVERY_SUMMARY_SCHEMA = "goldendict-real-dictionary-recovery-summary-v1"
CANCELLATION_SUMMARY_SCHEMA = "goldendict-real-dictionary-cancellation-summary-v1"
...
def _indexes_for_role(
    observation: dict[str, object], role: str
) -> list[dict[str, object]]:
    return [item for item in observation["indexes"] if item["role"] == role]
...
def _validate_recovery_transitions(
    observations: list[dict[str, object]],
) -> None:
    if not observations:
        return
    for index, observation in enumerate(observations):
        if observation["scenario"] != RECOVERY_SCENARIOS[index]:
            raise LifecycleError("Recovery scenarios are not contiguous")

    clean = observations[0]
    if len(clean["dictionaries"]) != 1:
        raise LifecycleError("Recovery lifecycle requires one fixture dictionary")
    if not _index_identities(clean) or _index_dispositions(clean) != {"created"}:
        raise LifecycleError("Recovery clean discovery must create indexes")

    if len(observations) >= 2:
        changed = observations[1]
        if changed["dictionaries"] != clean["dictionaries"]:
            raise LifecycleError("Changed source altered dictionary identity")
        if _index_shape(changed) != _index_shape(clean):
            raise LifecycleError("Changed source altered index ownership")
        if _index_dispositions(changed) != {"rebuilt"}:
            raise LifecycleError("Changed source must rebuild every fixture index")

    if len(observations) >= 3:
        unavailable = observations[2]
        if unavailable["dictionaries"] or unavailable["indexes"]:
            raise LifecycleError(
                "Unavailable companion must remove the fixture dictionary"
            )

    if len(observations) >= 4:
        changed = observations[1]
        recovered = observations[3]
        if recovered["dictionaries"] != clean["dictionaries"]:
            raise LifecycleError("Restored companion did not recover the dictionary")
        if _index_identities(recovered) != _index_identities(changed):
            raise LifecycleError("Companion recovery changed current index artifacts")
        if _index_dispositions(recovered) != {"created"}:
            raise LifecycleError("Companion recovery must recreate removed indexes")


def _validate_cancellation_transitions(
    observations: list[dict[str, object]],
) -> None:
    if not observations:
        return
    for index, observation in enumerate(observations):
        if observation["scenario"] != CANCELLATION_SCENARIOS[index]:
            raise LifecycleError("Cancellation scenarios are not contiguous")
        if len(observation["dictionaries"]) != 1:
            raise LifecycleError(
                "Cancellation lifecycle requires one fixture dictionary"
            )

    cancelled = observations[0]
    if cancelled["outcome"] != "cancelled":
        raise LifecycleError("Cancellation observation must be cancelled")
    if _indexes_for_role(cancelled, "full-text"):
        raise LifecycleError("Cancelled indexing must not publish a full-text index")
    if [item["status"] for item in cancelled["phases"]] != [
        "started",
        "cancelled",
    ]:
        raise LifecycleError("Cancellation progress is incomplete")

    if len(observations) >= 2:
        recovered = observations[1]
        if recovered["dictionaries"] != cancelled["dictionaries"]:
            raise LifecycleError("Cancellation recovery changed dictionary identity")
        full_text_indexes = _indexes_for_role(recovered, "full-text")
        if not full_text_indexes or {
            item["disposition"] for item in full_text_indexes
        } != {"created"}:
            raise LifecycleError(
                "Cancellation recovery must create the full-text index"
            )
        if recovered["outcome"] != "completed" or [
            item["status"] for item in recovered["phases"]
        ] != ["started", "completed"]:
            raise LifecycleError("Cancellation recovery progress is incomplete")
    ...
    *,
    ...
    for scenario in RECOVERY_SCENARIOS:
        ...
        if not path.exists():
            ...
            continue
        if missing_seen:
            raise LifecycleError("Recovery evidence contains a scenario gap")
        if path.is_symlink() or _is_reparse_point(path) or not path.is_file():
            raise LifecycleError("Recovery archive must be a regular workspace file")
        observation = acceptance_result.read_observation(
            path,
            expected_pair_id=pair_id,
            expected_version=version,
        ...
        if observation["scenario"] != scenario:
            raise LifecycleError("Recovery archive scenario does not match its name")
        observations.append(observation)
    _validate_recovery_transitions(observations)
    return observations


def run_recovery_version(
    workspace: Path,
    corpus: Path,
    manifest: Path,
    conditions: Path,
    ...
    if version not in acceptance_result.VERSIONS:
        raise LifecycleError("Recovery lifecycle version must be qt5 or qt6")
    _scenario_command(command, RECOVERY_SCENARIOS[0])
    workspace_root = workspace.resolve(strict=True)
    revision = _expected_revision(version, qt5_revision, qt6_revision)
    evidence_root = workspace_root / version / "evidence"
    observation_path = evidence_root / "observation.json"
    archive_root = _confined_output_directory(
        evidence_root, "recovery", "Recovery archive"
    )
    manifest_hash, conditions_hash = acceptance_workspace.read_pair_hashes(
        workspace_root, pair_id
    )
    archives = _read_recovery_archives(
        archive_root,
        pair_id=pair_id,
        version=version,
        revision=revision,
    ...
    for scenario in RECOVERY_SCENARIOS[len(archives) :]:
        return_code = acceptance_workspace.run_in_workspace(
            workspace,
            corpus,
            manifest,
        ...
        if observation["scenario"] != scenario:
            raise LifecycleError("Observer published the wrong recovery scenario")
        archives.append(observation)
        _validate_recovery_transitions(archives)
        _archive_observation(observation_path, archive_root / f"{scenario}.json")

    return [archive_root / f"{scenario}.json" for scenario in RECOVERY_SCENARIOS]


def _read_cancellation_archives(
    directory: Path,
    *,
    pair_id: str,
    version: str,
    ...
    for scenario in CANCELLATION_SCENARIOS:
        ...
        if not path.exists():
            ...
            continue
        if missing_seen:
            raise LifecycleError("Cancellation evidence contains a scenario gap")
        if path.is_symlink() or _is_reparse_point(path) or not path.is_file():
            raise LifecycleError(
                "Cancellation archive must be a regular workspace file"
            )
        observation = acceptance_result.read_observation(
            path,
            expected_pair_id=pair_id,
        ...
        if observation["scenario"] != scenario:
            raise LifecycleError(
                "Cancellation archive scenario does not match its name"
            )
        observations.append(observation)
    _validate_cancellation_transitions(observations)
    return observations


def run_cancellation_version(
    workspace: Path,
    corpus: Path,
    manifest: Path,
    conditions: Path,
    ...
    if version not in acceptance_result.VERSIONS:
        raise LifecycleError("Cancellation lifecycle version must be qt5 or qt6")
    _scenario_command(command, CANCELLATION_SCENARIOS[0])
    workspace_root = workspace.resolve(strict=True)
    revision = _expected_revision(version, qt5_revision, qt6_revision)
    evidence_root = workspace_root / version / "evidence"
    observation_path = evidence_root / "observation.json"
    archive_root = _confined_output_directory(
        evidence_root, "cancellation", "Cancellation archive"
    )
    manifest_hash, conditions_hash = acceptance_workspace.read_pair_hashes(
        workspace_root, pair_id
    )
    archives = _read_cancellation_archives(
        archive_root,
        pair_id=pair_id,
        version=version,
        revision=revision,
    ...
    for scenario in CANCELLATION_SCENARIOS[len(archives) :]:
        ...
        if observation["scenario"] != scenario:
            raise LifecycleError("Observer published the wrong cancellation scenario")
        archives.append(observation)
        _validate_cancellation_transitions(archives)
        _archive_observation(observation_path, archive_root / f"{scenario}.json")

    return [archive_root / f"{scenario}.json" for scenario in CANCELLATION_SCENARIOS]
...
def _validate_recovery_summary(value: object) -> dict[str, object]:
    if not isinstance(value, dict) or set(value) != {
        "equivalent",
        "pair_id",
        "scenarios",
        ...
        raise LifecycleError("Recovery summary has an unexpected structure")
    if (
        value["schema"] != RECOVERY_SUMMARY_SCHEMA
        or not isinstance(value["pair_id"], str)
        or not acceptance_result.HASH_PATTERN.fullmatch(value["pair_id"])
    ):
        raise LifecycleError("Recovery summary identity is invalid")
    records = value["scenarios"]
    if not isinstance(records, list) or len(records) != len(RECOVERY_SCENARIOS):
        raise LifecycleError("Recovery summary must contain every scenario")
    total = 0
    for index, record in enumerate(records):
        if not isinstance(record, dict) or set(record) != {
            "comparison",
            ...
            raise LifecycleError("Recovery summary scenario is malformed")
        scenario = RECOVERY_SCENARIOS[index]
        count = record["difference_count"]
        if (
            record["scenario"] != scenario
            or record["comparison"] != f"recovery-comparisons/{scenario}.json"
            or not isinstance(count, int)
            or isinstance(count, bool)
            or count < 0
            or record["equivalent"] != (count == 0)
        ):
            raise LifecycleError("Recovery summary scenario is inconsistent")
        total += count
    if value["total_difference_count"] != total or value["equivalent"] != (total == 0):
        raise LifecycleError("Recovery summary totals are inconsistent")
    return value
...
def compare_recovery(
    ...
    archives = {
        version: _read_recovery_archives(
            workspace_root / version / "evidence" / "recovery",
            pair_id=pair_id,
            version=version,
            revision=_expected_revision(version, qt5_revision, qt6_revision),
            manifest_hash=manifest_hash,
    ...
    if any(len(values) != len(RECOVERY_SCENARIOS) for values in archives.values()):
        raise LifecycleError("Both versions must complete every recovery scenario")

    comparison_root = _confined_output_directory(
        workspace_root, "recovery-comparisons", "Recovery comparison"
    )
    records: list[dict[str, object]] = []
    total_differences = 0
    for index, scenario in enumerate(RECOVERY_SCENARIOS):
        comparison = acceptance_result.compare_observations(
            archives["qt5"][index], archives["qt6"][index]
        )
        output = comparison_root / f"{scenario}.json"
        ...
        records.append(
            {
                "comparison": f"recovery-comparisons/{scenario}.json",
                "difference_count": difference_count,
                "equivalent": comparison["equivalent"],
                "scenario": scenario,
            }
    ...
    summary_path = comparison_root / "recovery-summary.json"
    content = acceptance_result.canonical_json(_validate_recovery_summary(summary))
    temporary: Path | None = None
    try:
        descriptor, temporary_name = tempfile.mkstemp(
        ...
        raise LifecycleError(
            f"Cannot publish recovery summary: {summary_path}"
        ) from error
    finally:
        if temporary is not None:
            temporary.unlink(missing_ok=True)
...
def _validate_cancellation_summary(value: object) -> dict[str, object]:
    if not isinstance(value, dict) or set(value) != {
        "equivalent",
        "pair_id",
        "scenarios",
        ...
        raise LifecycleError("Cancellation summary has an unexpected structure")
    if (
        value["schema"] != CANCELLATION_SUMMARY_SCHEMA
        or not isinstance(value["pair_id"], str)
        or not acceptance_result.HASH_PATTERN.fullmatch(value["pair_id"])
    ):
        raise LifecycleError("Cancellation summary identity is invalid")
    records = value["scenarios"]
    if not isinstance(records, list) or len(records) != len(CANCELLATION_SCENARIOS):
        raise LifecycleError("Cancellation summary must contain every scenario")
    total = 0
    for index, record in enumerate(records):
        if not isinstance(record, dict) or set(record) != {
            "comparison",
            ...
            raise LifecycleError("Cancellation summary scenario is malformed")
        scenario = CANCELLATION_SCENARIOS[index]
        count = record["difference_count"]
        if (
            record["scenario"] != scenario
            or record["comparison"] != f"cancellation-comparisons/{scenario}.json"
            or not isinstance(count, int)
            or isinstance(count, bool)
            or count < 0
            or record["equivalent"] != (count == 0)
        ):
            raise LifecycleError("Cancellation summary scenario is inconsistent")
        total += count
    if value["total_difference_count"] != total or value["equivalent"] != (total == 0):
        raise LifecycleError("Cancellation summary totals are inconsistent")
    return value


def compare_cancellation(
    workspace: Path,
    corpus: Path,
    manifest: Path,
    conditions: Path,
    ...
    archives = {
        version: _read_cancellation_archives(
            workspace_root / version / "evidence" / "cancellation",
            pair_id=pair_id,
            version=version,
            revision=_expected_revision(version, qt5_revision, qt6_revision),
            manifest_hash=manifest_hash,
    ...
    if any(len(values) != len(CANCELLATION_SCENARIOS) for values in archives.values()):
        raise LifecycleError("Both versions must complete every cancellation scenario")

    comparison_root = _confined_output_directory(
        workspace_root, "cancellation-comparisons", "Cancellation comparison"
    )
    records: list[dict[str, object]] = []
    total_differences = 0
    for index, scenario in enumerate(CANCELLATION_SCENARIOS):
        comparison = acceptance_result.compare_observations(
            archives["qt5"][index], archives["qt6"][index]
        )
        output = comparison_root / f"{scenario}.json"
        ...
        records.append(
            {
                "comparison": f"cancellation-comparisons/{scenario}.json",
                "difference_count": difference_count,
                "equivalent": comparison["equivalent"],
                "scenario": scenario,
            }
    ...
    summary = {
        ...
        "pair_id": pair_id,
        "scenarios": records,
        "schema": CANCELLATION_SUMMARY_SCHEMA,
        "total_difference_count": total_differences,
    }
    summary_path = comparison_root / "cancellation-summary.json"
    content = acceptance_result.canonical_json(_validate_cancellation_summary(summary))
    temporary: Path | None = None
    try:
        descriptor, temporary_name = tempfile.mkstemp(
            prefix=f".{summary_path.name}.", dir=summary_path.parent
        ...
        raise LifecycleError(
            f"Cannot publish cancellation summary: {summary_path}"
        ) from error
    finally:
        if temporary is not None:
            temporary.unlink(missing_ok=True)
...
def _parser() -> argparse.ArgumentParser:
    ...
    recovery = subparsers.add_parser("run-recovery-version")
    _common_arguments(recovery)
    recovery.add_argument(
        "--version", choices=acceptance_result.VERSIONS, required=True
    )
    recovery.add_argument("child_command", nargs=argparse.REMAINDER)
    cancellation = subparsers.add_parser("run-cancellation-version")
    _common_arguments(cancellation)
    cancellation.add_argument(
        "--version", choices=acceptance_result.VERSIONS, required=True
    )
    cancellation.add_argument("child_command", nargs=argparse.REMAINDER)
    compare = subparsers.add_parser("compare")
    _common_arguments(compare)
    compare_recovery_parser = subparsers.add_parser("compare-recovery")
    _common_arguments(compare_recovery_parser)
    compare_cancellation_parser = subparsers.add_parser("compare-cancellation")
    _common_arguments(compare_cancellation_parser)
    return parser
...
def main(arguments: Iterable[str] | None = None) -> int:
    ...
    try:
        if options.command in (
            "run-version",
            "run-recovery-version",
            "run-cancellation-version",
        ):
            ...
            runner = {
                ...
                "run-recovery-version": run_recovery_version,
                "run-cancellation-version": run_cancellation_version,
            }[options.command]
            outputs = runner(
                options.workspace,
                options.corpus,
        ...
        elif options.command == "compare-recovery":
            print(
                compare_recovery(
                    options.workspace,
                    options.corpus,
