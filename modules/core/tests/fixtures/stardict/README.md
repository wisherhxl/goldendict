...
The fixture covers uncompressed StarDict 2.4.2 files with 32-bit index offsets,
plain `sametypesequence=m` articles, raw HTML `sametypesequence=h` articles, an
internal dictionary link, and a resource under the adjacent `res` directory.
Compressed data, synonyms, resource ZIP archives, and other article type
sequences require separate compatibility fixtures before they are enabled.
