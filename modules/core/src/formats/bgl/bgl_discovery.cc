...
#include "bgl_discovery.h"
#include <algorithm>
#include <cctype>

namespace goldendict::core::formats::bgl {
namespace {
bool IsBgl(const std::filesystem::path& path) {
    std::string name = path.filename().string();
    std::transform(name.begin(), name.end(), name.begin(), [](unsigned char c) {
        return static_cast<char>(std::tolower(c));
    });
    return name.size() >= 4U && name.compare(name.size() - 4U, 4U, ".bgl") == 0;
}
}  // namespace
...
DiscoveryResult Discover(const std::vector<std::filesystem::path>& roots) {
    ...
    for (const auto& root : roots) {
        ...
        while (!error && it != end) {
            ...
            if (it->is_regular_file(error) && !error && IsBgl(path))
