...
[[noreturn]] void Throw(ZipArchiveErrorCode code,
                        const std::filesystem::path& path,
                        std::string message) {
    throw ZipArchiveError(code, path.string() + ": " + std::move(message));
}

std::uint16_t Le16(std::string_view data, std::size_t offset,
                   const std::filesystem::path& path) {
    if (offset > data.size() || data.size() - offset < 2U)
        Throw(ZipArchiveErrorCode::kInvalidData, path, "Truncated ZIP integer");
...
std::uint64_t Le64(std::string_view data, std::size_t offset,
    ...
    if (offset > data.size() || data.size() - offset < 8U)
        Throw(ZipArchiveErrorCode::kInvalidData, path,
...
std::string DecodeName(std::string_view bytes, bool utf8,
                       std::size_t maximum_name_size,
                       const std::filesystem::path& path) {
    try {
        if (utf8)
            return DecodeToUtf8(bytes, "UTF-8", maximum_name_size);
        try {
            return DecodeToUtf8(bytes, "UTF-8", maximum_name_size);
        } catch (const foundation::TextEncodingError&) {
            return DecodeToUtf8(bytes, "IBM437", maximum_name_size);
        }
    } catch (const foundation::TextEncodingError&) {
        Throw(ZipArchiveErrorCode::kInvalidData, path,
...
DirectoryLocation LocateDirectory(std::ifstream& input, std::uint64_t file_size,
    ...
    if (file_size < 22U)
        Throw(ZipArchiveErrorCode::kInvalidData, path, "Truncated ZIP archive");
    ...
    if (location.entries > limits.maximum_entries ||
        location.size > limits.maximum_central_size ||
        location.offset > file_size ||
...
ZipArchiveError::ZipArchiveError(ZipArchiveErrorCode code, std::string message)
    : std::runtime_error(std::move(message)), code_(code) {}

ZipArchive ZipArchive::Open(const std::filesystem::path& archive_path,
    ...
    while (cursor < central.size()) {
        if (checkpoint)
            checkpoint();
        if (parsed_entries == limits.maximum_entries)
            Throw(ZipArchiveErrorCode::kInvalidData, result.path_,
        ...
        if (name_size == 0U || name_size > limits.maximum_name_size ||
        ...
        if ((!zip64_disk && disk != 0U) || (flags & 1U) != 0U ||
            (method != 0U && method != 8U) ||
            uncompressed > limits.maximum_resource_size ||
            compressed > limits.maximum_compressed_size)
            continue;
        std::string resource_id =
            DecodeName(variable.substr(0U, name_size), (flags & 0x800U) != 0U,
                       limits.maximum_name_size, result.path_);
        std::replace(resource_id.begin(), resource_id.end(), '\\', '/');
        if (!SafeResourceId(resource_id))
            continue;
        if (limits.normalize_resource_id)
            resource_id = limits.normalize_resource_id(resource_id);
    ...
    if (std::filesystem::file_size(result.path_, error) !=
        ...
        error)
        Throw(ZipArchiveErrorCode::kUnavailable, result.path_,
              "ZIP archive changed during discovery");
    if (checkpoint)
        checkpoint();
    return result;
}

std::optional<std::vector<std::byte>> ZipArchive::Read(
    ...
    if (!SafeResourceId(resource_id))
        return std::nullopt;
    const auto key = limits_.normalize_resource_id
                         ? limits_.normalize_resource_id(resource_id)
                         : std::string(resource_id);
    ...
    if (std::filesystem::file_size(path_, error) != source_size_ || error ||
        ...
        error)
        Throw(ZipArchiveErrorCode::kUnavailable, path_,
              "ZIP archive changed after discovery");
    std::ifstream input(path_, std::ios::binary);
    if (!input)
        Throw(ZipArchiveErrorCode::kUnavailable, path_,
              "Cannot reopen ZIP archive");
    const Entry& entry = found->second;
    if (entry.local_offset > central_offset_ ||
        central_offset_ - entry.local_offset < 30U)
        Throw(ZipArchiveErrorCode::kInvalidData, path_,
    ...
    if (std::filesystem::file_size(path_, error) != source_size_ || error ||
        ...
        error)
        Throw(ZipArchiveErrorCode::kUnavailable, path_,
              "ZIP archive changed during reading");
    if (checkpoint)
        checkpoint();
    return output;
}
