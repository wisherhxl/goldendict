...
class SynonymPreferencesTest : public QObject {
    Q_OBJECT
   public:
    explicit SynonymPreferencesTest(QString owned_root)
   ...
   private slots:
    ...
    void scenarioThroughRealApplication() {
        ...
        ArticlesPreferencesTestAccess::SetDialogExecutor(
            window, [&passed, initial_preferences](PreferencesDialog& dialog) {
                ...
                passed =
                    ...
                    checkbox->text() ==
                        QStringLiteral("Extra search via synonyms") &&
                    checkbox->toolTip() ==
                        QStringLiteral(
                            "Turn this option on to enable extra articles "
                            "search "
                            "via synonym lists from Stardict, Babylon and GLS "
                            "dictionaries") &&
                    checkbox->isChecked() ==
                        initial_preferences.synonym_search_enabled;
                if (checkbox != nullptr)
                    checkbox->setChecked(
                        !initial_preferences.synonym_search_enabled);
                dialog.reject();
        ...
        passed =
            passed &&
            !ViewMenuTestAccess::Preferences(window).synonym_search_enabled &&
        ...
        const auto persisted =
            core::LoadConfiguration(configuration_path.toStdString());
        QCOMPARE(persisted.preferences.synonym_search_enabled, false);
    }
};

int main(int argc, char** argv) {
    QTemporaryDir profile(QDir::tempPath() + "/sy-XXXXXX");
    if (!profile.isValid())
        return 2;
    for (const auto* name :
    ...
    SynonymPreferencesTest test(profile.path());
    return QTest::qExec(&test, argc, argv);
}

#include "synonym_preferences_test.moc"
