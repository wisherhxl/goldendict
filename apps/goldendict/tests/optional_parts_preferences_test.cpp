...
class OptionalPartsPreferencesTest : public QObject {
    Q_OBJECT
   public:
    explicit OptionalPartsPreferencesTest(QString owned_root)
   ...
   private slots:
    ...
    void scenarioThroughRealApplication() {
        ...
        ArticlesPreferencesTestAccess::SetDialogExecutor(
            window, [&inspect, initial_preferences](PreferencesDialog& dialog) {
                return inspect(
                    dialog, initial_preferences.always_expand_optional_parts,
                    !initial_preferences.always_expand_optional_parts, false);
            });
        ...
        preferences_action_->trigger();
        passed = passed && ViewMenuTestAccess::Preferences(window)
        ...
        passed = passed &&
                 ...
                 ArticlesPreferencesTestAccess::ArticleTabsVisible(window);
        QVERIFY(passed);
        QVERIFY(published());
...
int main(int argc, char** argv) {
    QTemporaryDir profile(QDir::tempPath() + "/op-XXXXXX");
    if (!profile.isValid())
        return 2;
    for (const auto* name :
    ...
    OptionalPartsPreferencesTest test(profile.path());
    int test_result = 2;
