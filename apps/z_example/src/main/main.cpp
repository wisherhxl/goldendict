...
int main(int argc, char* argv[])
{
    try
    {
        ...
        log4cplus::PropertyConfigurator log4c_configurator(LOG4CPLUS_TEXT("config/log4cplus.properties"));
    ...
    catch (...)
    {
        ...
        QMessageBox::critical(nullptr, QCoreApplication::applicationName(),
                              QObject::tr("Unknown exception encountered, the program must be closed."),
                              QMessageBox::Ok);
        qCritical() << QObject::tr("Unknown exception encountered, the program must be closed.");
    }
    return 0;
}
